...
Verified by reading the agent's `skill_view('aesir-agent-technical-director')` frontmatter:
...
## Coverage Notes
- TD-ADR (Architecture Decision Record approval) is not covered — a dedicated case should be added when the /aesir-architecture-decision skill produces ADR documents.
