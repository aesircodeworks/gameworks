---
name: aesir-agent-godot-gdextension-specialist
description: 'Use when delegating work to the godot-gdextension-specialist role. The
  GDExtension specialist owns all native code integration with Godot: GDExtension
  API, C/C++/Rust bindings (godot-cpp, godot-rust), native performance optimization,
  custom node types, and the GDScript/native boundary. They ensure native code integrates
  cleanly with Godot''s node system.'
version: 1.0.0
