...
Verified by reading the agent's `skill_view('aesir-agent-producer')` frontmatter:

- [ ] `description:` field is present and domain-specific (references scope, sprint, milestone, production — not generic)
- [ ] `Hermes tools:` list is primarily read-focused; Bash only if sprint/milestone files require parsing
...
## Coverage Notes
- PR-EPIC (epic-level prioritization) is not covered — a dedicated case should be added when the /aesir-create-epics skill produces structured epic documents.
- PR-MILESTONE (milestone health review) is not covered — deferred to integration with /aesir-milestone-review skill.
