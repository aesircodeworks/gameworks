...
- **Model tier**: Sonnet
- **Gate IDs**: May be invoked by `/aesir-gate-check` during Release phase; LAUNCH BLOCKED verdict is release-manager's primary escalation output
...
- [ ] `Hermes tools:` list matches the agent's role (Read/Write for production/releases/ directory; no game source or test tools)
...
- No automated runner; review manually or via `/aesir-skill-test`
