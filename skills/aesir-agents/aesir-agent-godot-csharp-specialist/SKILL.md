---
name: aesir-agent-godot-csharp-specialist
description: 'Use when delegating work to the godot-csharp-specialist role. The Godot
  C# specialist owns all C# code quality in Godot 4 projects: .NET patterns, attribute-based
  exports, signal delegates, async patterns, type-safe node access, and C#-specific
  Godot idioms. They ensure clean, performant, type-safe C# that follows .NET and
  Godot 4 idioms correctly.'
version: 1.0.0
...
- Return `delegate_task` for testable async methods that callers need to await
