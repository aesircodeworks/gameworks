...
Verified by reading the agent's `skill_view('aesir-agent-qa-lead')` frontmatter:
...
- Bug severity triage (P0/P1/P2 classification) is not covered here — deferred to /aesir-bug-triage skill integration.
- Release quality gate behavior (PASS / FAIL vocabulary variant) is not covered.
- Interaction between QL-STORY-READY and story Done criteria (/aesir-story-done skill) is not covered.
