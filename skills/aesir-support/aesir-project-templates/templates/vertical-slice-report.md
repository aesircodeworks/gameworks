...
**Playtest note:** Run `/aesir-playtest-report` to structure additional session data
before running `/aesir-gate-check pre-production`.

**Next steps:**
1. `/aesir-gate-check pre-production` — formally advance to Production
2. `/aesir-create-epics layer:foundation` — plan Foundation layer epics
3. `/aesir-create-epics layer:core` — plan Core layer epics
4. `/aesir-sprint-plan` — use velocity data from this report in the estimate
...
**Architecture decisions to revisit:** [List — use `/aesir-architecture-decision` to update]
**Core loop change needed:** [What specifically to change]

**Next steps:**
1. `/aesir-design-system [mechanic]` — revise affected GDDs
2. `/aesir-architecture-decision [decision]` — address architecture issues
3. `/aesir-vertical-slice` — re-validate after revisions
...
**Next step:** `/aesir-brainstorm` to explore a new direction, or `/aesir-prototype [new-concept]`
