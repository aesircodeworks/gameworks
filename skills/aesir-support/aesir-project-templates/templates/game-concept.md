...
- [ ] Fill in CLAUDE.md technology stack based on engine choice (`/aesir-setup-engine`)
- [ ] Create game pillars document (`/aesir-design-review` to validate)
- [ ] **Prototype core idea** (`/aesir-prototype [core-mechanic]`) — before writing GDDs, validate the concept is worth designing
- [ ] If prototype PROCEEDS: Decompose concept into systems (`/aesir-map-systems`)
- [ ] Design each system (`/aesir-design-system [system-name]`) — use prototype learnings in Tuning Knobs and Formulas sections
- [ ] Build vertical slice in Pre-Production (`/aesir-vertical-slice`) — validate full game loop before committing to Production
- [ ] Validate core loop with playtest (`/aesir-playtest-report`)
- [ ] Plan first milestone (`/aesir-sprint-plan new`)
