...
- [ ] Design MVP-tier systems first (use `/aesir-design-system [system-name]`)
- [ ] Run `/aesir-design-review` on each completed GDD
- [ ] Run `/aesir-gate-check pre-production` when MVP systems are designed
- [ ] Validate the highest-risk systems with `/aesir-vertical-slice` before committing to Production
