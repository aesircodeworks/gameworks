...
Apply gate **TD-ARCHITECTURE** (`skill_view('gameworks', file_path='references/director-gates.md')`) as a self-review. Check all four criteria from that gate definition against the completed document.
...
**Step 2 — Spawn `lead-programmer` with delegate_task using gate LP-FEASIBILITY (`skill_view('gameworks', file_path='references/director-gates.md')`):**
