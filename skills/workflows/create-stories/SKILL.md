---
name: create-stories
description: Break an epic into implementable stories.
version: 1.0.0
...
in this skill — apply the check pattern from `skill_view('gameworks', file_path='references/director-gates.md')`
...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `qa-lead` with delegate_task using gate **QL-STORY-READY** (`skill_view('gameworks', file_path='references/director-gates.md')`).
