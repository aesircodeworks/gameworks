...
After completing the deviation checks in Phase 4, spawn `qa-lead` with delegate_task using gate **QL-TEST-COVERAGE** (`skill_view('gameworks', file_path='references/director-gates.md')`).
...
Spawn `lead-programmer` with delegate_task using gate **LP-CODE-REVIEW** (`skill_view('gameworks', file_path='references/director-gates.md')`).
