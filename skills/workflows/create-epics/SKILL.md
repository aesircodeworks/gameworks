...
After all epics for the current layer are defined (Step 4 completed for all in-scope systems), and before writing any files, spawn `producer` with delegate_task using gate **PR-EPIC** (`skill_view('gameworks', file_path='references/director-gates.md')`).
