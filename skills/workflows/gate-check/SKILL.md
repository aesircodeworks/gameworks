...
- [ ] Vertical Slice playtest report exists at `production/qa/playtests/` — **recommended, not blocking**; if absent, surface as CONCERNS
...
Use `search_files` with `file_glob="design/gdd/gdd-cross-review-*.md"` to find the `/review-all-gdds` report.
...
- For performance checks: `read_file` `docs/technical-preferences.md` and compare against any
...
**Collect all four responses.** Parse the first line of each for `[GATE-ID]: TOKEN` (`READY` / `CONCERNS` / `NOT READY`). Then present the Director Panel summary:
...
> **Tool-action requirement**: At least 2 of the 5 challenge questions below must be answered by re-reading a specific file (`read_file`) or re-running a specific check (`search_files`) — not by reflection alone. Mark these with [TOOL ACTION] to indicate a tool was used.
