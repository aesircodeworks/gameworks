...
This folder is the quality assurance layer for the Aesir Gameworks skill/agent
...
| `quality-rubric.md` | Category-specific pass/fail metrics. Read the matching `###` section for the skill's category when running `/aesir-skill-test category`. |
...
| `results/` | Written by `/aesir-skill-test spec` when results are saved. Gitignored. |

## Path conventions

- Skill specs: `aesir-workflows behavior specs: [category]/[name].md`
- Agent specs: `aesir-framework-qa references/agents/[tier]/[name].md`
- Catalog: `aesir-framework-qa references/catalog.yaml`
- Rubric: `aesir-framework-qa references/quality-rubric.md`
...
2. Read the skill at `(game-workspace)/skills/[name]/SKILL.md`
...
Use `/aesir-skill-improve [name]`. It handles the full loop:
...
Nothing in `(game-workspace)/` imports from here. Deleting this folder has no effect on the
CCGS skills or agents themselves. `/aesir-skill-test` and `/aesir-skill-improve` will report that
