...
Quality assurance infrastructure for the **Aesir Gameworks** framework.
...
> `rm -rf "CCGS Skill Testing Framework"` — nothing in `(game-workspace)/` depends on it.
...
- `/aesir-skill-test spec [name]` will offer to update `last_spec` and `last_spec_result`
- `/aesir-skill-test category [name]` will offer to update `last_category` and `last_category_result`
- `last_static` and `last_static_result` are updated manually or via `/aesir-skill-improve`
...
4. Run `/aesir-skill-test spec [skill-name]` to validate it
...
The skills `/aesir-skill-test` and `/aesir-skill-improve` will still function — they'll simply
report that `catalog.yaml` is missing and suggest running `/aesir-skill-test audit` to
initialize it.
