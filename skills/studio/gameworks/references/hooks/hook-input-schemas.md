...
This documents the JSON payloads each Hermes Agent hook receives on stdin for every event type.
...
Fired when a subagent is spawned via the delegate_task tool.
...
Fired when a Hermes Agent session begins. **No stdin input** — the hook just runs and its stdout is shown to Claude as context.
...
Fired when the Hermes Agent session ends. **No stdin input** — the hook runs for cleanup and logging.
