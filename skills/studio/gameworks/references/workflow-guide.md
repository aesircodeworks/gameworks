...
`/story-done` (status updates). Read by `/sprint-status`, `/studio-help`, and
...
controls the status line and `/studio-help` behavior.
...
```
1. /studio-start (routes you based on where you are)
...
```
1. /studio-start (choose Path D -- existing work)
...
```
1. /studio-help (reads your phase, checks artifacts, tells you what's next)
2. If /studio-help doesn't help: /project-stage-detect (full audit)
...
6. **Run /studio-help when uncertain.** It reads your actual project state and tells
