...
| Brainstorm a new game idea | Use `/aesir-brainstorm` skill |

### 3. Use Slash Commands for Common Tasks

| Command | What it does |
|---------|-------------|
| `/aesir-start` | First-time onboarding — asks where you are, guides you to the right workflow |
| `/aesir-help` | Context-aware "what do I do next?" — reads your current phase and artifacts |
| `/aesir-project-stage-detect` | Analyze project state, detect stage, identify gaps |
| `/aesir-setup-engine` | Configure engine + version, populate reference docs |
| `/aesir-adopt` | Brownfield audit and migration plan for existing projects |
| `/aesir-brainstorm` | Guided game concept ideation from scratch |
| `/aesir-map-systems` | Decompose concept into systems, map dependencies, guide per-system GDDs |
| `/aesir-design-system` | Guided, section-by-section GDD authoring for a single game system |
| `/aesir-quick-design` | Lightweight spec for small changes — tuning, tweaks, minor additions |
| `/aesir-review-all-gdds` | Cross-GDD consistency and game design theory review |
| `/aesir-propagate-design-change` | Find ADRs and stories affected by a GDD change |
| `/aesir-art-bible` | Guided, section-by-section Art Bible authoring — creates visual identity spec before asset production |
| `/aesir-asset-spec` | Generate per-asset visual specifications and AI generation prompts from GDDs or character profiles |
| `/aesir-ux-design` | Author UX specs (screen/flow, HUD, interaction patterns) |
| `/aesir-ux-review` | Validate UX specs for accessibility and GDD alignment |
| `/aesir-create-architecture` | Master architecture document for the game |
| `/aesir-architecture-decision` | Creates an ADR |
| `/aesir-architecture-review` | Validate all ADRs, dependency ordering, GDD traceability |
| `/aesir-create-control-manifest` | Flat programmer rules sheet from Accepted ADRs |
...
| `/aesir-sprint-plan` | Creates or updates sprint plans |
| `/aesir-sprint-status` | Quick 30-line sprint snapshot |
| `/aesir-story-readiness` | Validate a story is implementation-ready before pickup |
| `/aesir-story-done` | End-of-story completion review — verifies acceptance criteria |
| `/aesir-estimate` | Produces structured effort estimates |
| `/aesir-design-review` | Reviews a design document |
| `/aesir-code-review` | Reviews code for quality and architecture |
| `/aesir-balance-check` | Analyzes game balance data |
| `/aesir-asset-audit` | Audits assets for compliance |
| `/aesir-content-audit` | GDD-specified content vs. implemented — find gaps |
| `/aesir-scope-check` | Detect scope creep against plan |
| `/aesir-perf-profile` | Performance profiling and bottleneck ID |
| `/aesir-tech-debt` | Scan, track, and prioritize tech debt |
| `/aesir-gate-check` | Validate phase readiness (PASS/CONCERNS/FAIL) |
| `/aesir-consistency-check` | Scan all GDDs for cross-document inconsistencies (conflicting stats, names, rules) |
| `/aesir-security-audit` | Audit for security vulnerabilities: save tampering, cheat vectors, network exploits, data exposure |
| `/aesir-reverse-document` | Generate design/architecture docs from existing code |
| `/aesir-milestone-review` | Reviews milestone progress |
| `/aesir-retrospective` | Runs sprint/milestone retrospective |
| `/aesir-bug-report` | Structured bug report creation |
| `/aesir-playtest-report` | Creates or analyzes playtest feedback |
| `/aesir-onboard` | Generates onboarding docs for a role |
| `/aesir-release-checklist` | Validates pre-release checklist |
| `/aesir-launch-checklist` | Complete launch readiness validation |
| `/aesir-changelog` | Generates changelog from git history |
| `/aesir-patch-notes` | Generate player-facing patch notes |
| `/aesir-hotfix` | Emergency fix with audit trail |
| `/aesir-day-one-patch` | Prepare a focused day-one patch for known issues discovered after gold master |
| `/aesir-prototype` | Concept prototype — validate core idea before writing GDDs (Phase 1) |
| `/aesir-vertical-slice` | Production-quality end-to-end build — validate full game loop (Phase 4) |
| `/aesir-localize` | Localization scan, extract, validate |
| `/aesir-team-combat` | Orchestrate full combat team pipeline |
| `/aesir-team-narrative` | Orchestrate full narrative team pipeline |
| `/aesir-team-ui` | Orchestrate full UI team pipeline |
| `/aesir-team-release` | Orchestrate full release team pipeline |
| `/aesir-team-polish` | Orchestrate full polish team pipeline |
| `/aesir-team-audio` | Orchestrate full audio team pipeline |
| `/aesir-team-level` | Orchestrate full level creation pipeline |
| `/aesir-team-live-ops` | Orchestrate live-ops team for seasons, events, and post-launch content |
| `/aesir-team-qa` | Orchestrate full QA team cycle — test plan, test cases, smoke check, sign-off |
| `/aesir-qa-plan` | Generate a QA test plan for a sprint or feature |
| `/aesir-bug-triage` | Re-prioritize open bugs, assign to sprints, surface systemic trends |
| `/aesir-smoke-check` | Run critical path smoke test gate before QA hand-off (PASS/FAIL) |
| `/aesir-soak-test` | Generate a soak test protocol for extended play sessions |
| `/aesir-regression-suite` | Map coverage to GDD critical paths, flag gaps, maintain regression suite |
| `/aesir-test-setup` | Scaffold test framework + CI pipeline for the project's engine (run once) |
| `/aesir-test-helpers` | Generate engine-specific test helper libraries and factory functions |
| `/aesir-test-flakiness` | Detect flaky tests from CI history, flag for quarantine or fix |
| `/aesir-test-evidence-review` | Quality review of test files and manual evidence — ADEQUATE/INCOMPLETE/MISSING |
| `/aesir-skill-test` | Validate skill files for compliance and correctness (static / spec / audit) |
| `/aesir-skill-improve` | Improve a skill using a test-fix-retest loop — diagnose, propose fix, rewrite, verify |

### 4. Use Templates for New Documents

Templates are in `aesir-project-templates templates/`:
...
Also in `aesir-project-templates templates/collaborative-protocols/` (used by agents, not typically edited directly):

- `design-agent-protocol.md` -- question-options-draft-approval cycle for design agents
- `implementation-agent-protocol.md` -- story pickup through /aesir-story-done cycle for programming agents
...
**Don't know where to begin?** Run `/aesir-start`. It asks where you are and routes
...
1. **Run `/aesir-start`** (or `/aesir-brainstorm open`) — guided creative exploration:
...
2. **Set up the engine** — Run `/aesir-setup-engine` (uses the brainstorm recommendation)
   - Configures CLAUDE.md, detects knowledge gaps, populates reference docs
   - Creates `aesir-gameworks references/technical-preferences.md` with naming conventions,
     performance budgets, and engine-specific defaults
   - If the engine version is newer than the LLM's training data, it fetches
     current docs from the web so agents suggest correct APIs
3. **Validate the concept** — Run `/aesir-design-review design/gdd/game-concept.md`
4. **Decompose into systems** — Run `/aesir-map-systems` to map all systems and dependencies
5. **Design each system** — Run `/aesir-design-system [system-name]` (or `/aesir-map-systems next`)
   to write GDDs in dependency order
6. **Prototype the mechanic** — Run `/aesir-prototype [core-mechanic]` (1–3 days — before writing GDDs)
7. **Design each system** — Run `/aesir-design-system [system-name]` to write GDDs, informed by prototype findings
8. **Plan the first sprint** — After architecture and `/aesir-vertical-slice`, run `/aesir-sprint-plan new`
9. Start building
...
1. **Set up the engine** — Run `/aesir-setup-engine [engine] [version]`
   (e.g., `/aesir-setup-engine godot 4.6`) — also creates technical preferences
2. **Write the Game Pillars** — delegate to `creative-director`
3. **Decompose into systems** — Run `/aesir-map-systems` to enumerate systems and dependencies
4. **Design each system** — Run `/aesir-design-system [system-name]` for GDDs in dependency order
5. **Create the initial ADR** — Run `/aesir-architecture-decision`
6. **Create the first milestone** in `production/milestones/`
7. **Plan the first sprint** — Run `/aesir-sprint-plan new`
8. Start building
...
1. **Run `/aesir-setup-engine`** with no arguments — it will ask about your game's
...
1. **Run `/aesir-start`** (or `/aesir-project-stage-detect`) — analyzes what exists,
   identifies gaps, and recommends next steps
2. **Run `/aesir-adopt`** if you have existing GDDs, ADRs, or stories — audits
   internal format compliance and builds a numbered migration plan to fill gaps
   without overwriting your existing work
3. **Configure engine if needed** — Run `/aesir-setup-engine` if not yet configured
4. **Validate phase readiness** — Run `/aesir-gate-check` to see where you stand
5. **Plan the next sprint** — Run `/aesir-sprint-plan new`
