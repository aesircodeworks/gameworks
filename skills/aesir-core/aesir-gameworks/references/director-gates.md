...
Set once during `/aesir-start`. Edit the file directly to change it at any time.
...
| `lean` | PHASE-GATEs only (`/aesir-gate-check`) — per-skill gates skipped | **Default** — solo devs and small teams; directors review at milestones only |
...
| **CONCERNS [list]** | Issues present but not blocking. | Surface to user via `clarify` — options: `Revise flagged items` / `Accept and proceed` / `Discuss further` |
...
**Trigger**: After the systems index is written by `/aesir-map-systems` — validates the
...
**Trigger**: After playtest reports are generated (`/aesir-playtest-report`), or after
...
**Trigger**: Always at `/aesir-gate-check` — spawn in parallel with TD-PHASE-GATE and PR-PHASE-GATE
...
**Trigger**: After `/aesir-map-systems` Phase 3 dependency mapping is agreed but before
...
**Trigger**: After the master architecture document is drafted (`/aesir-create-architecture`
...
**Trigger**: After an individual ADR is authored (`/aesir-architecture-decision`), before
...
**Trigger**: Always at `/aesir-gate-check` — spawn in parallel with CD-PHASE-GATE and PR-PHASE-GATE
...
**Trigger**: Before finalising a sprint plan (`/aesir-sprint-plan`), and after any
...
**Trigger**: At milestone review (`/aesir-milestone-review`), at mid-sprint retrospectives,
...
**Trigger**: After epics are defined by `/aesir-create-epics`, before stories are
broken out — validates the epic structure is producible before `/aesir-create-stories`
is invoked
...
**Trigger**: Always at `/aesir-gate-check` — spawn in parallel with CD-PHASE-GATE and TD-PHASE-GATE
...
**Trigger**: After the art bible is drafted (`/aesir-art-bible`), before asset production begins

**Context to pass**:
- Art bible path (`design/art/art-bible.md`)
- Game pillars and core fantasy
- Platform and performance constraints (from `aesir-gameworks references/technical-preferences.md` if configured)
...
**Trigger**: Always at `/aesir-gate-check` — spawn in parallel with CD-PHASE-GATE, TD-PHASE-GATE, and PR-PHASE-GATE
...
**Trigger**: After the master architecture document is written (`/aesir-create-architecture`
...
**Trigger**: After a dev story is implemented (`/aesir-dev-story`, `/aesir-story-done`), or
as part of `/aesir-code-review`
...
**Trigger**: Before a story is accepted into a sprint — invoked by `/aesir-create-stories`,
`/aesir-story-readiness`, and `/aesir-sprint-plan` during story selection
...
done, or at `/aesir-gate-check` Production → Polish
...
at `/aesir-gate-check`), spawn all agents simultaneously:
