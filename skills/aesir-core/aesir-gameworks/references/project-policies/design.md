...
**Validation:** Run `/aesir-design-review [path]` after authoring any GDD.
Run `/aesir-review-all-gdds` after completing a set of related GDDs.

## Quick Specs (`design/quick-specs/`)

Lightweight specs for tuning changes, minor mechanics, or balance adjustments.
Use `/aesir-quick-design` to author.
...
Use `/aesir-ux-design` to author. Validate with `/aesir-ux-review` before passing to `/aesir-team-ui`.
