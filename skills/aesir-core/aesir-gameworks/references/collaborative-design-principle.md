...
**Example:** `/aesir-brainstorm roguelike`
...
## 🎛️ Structured Decision UI (clarify)

Use the `clarify` tool to present decisions as a **selectable UI** instead
...
- When running as a delegate_task subagent (tool may not be available)
...
(main session) calls `clarify` at each decision point between phases:
...
Team skills (like `/aesir-team-combat`) orchestrate multiple agents, but still collaborative:
...
- **clarify tool** — Integrated into 16 skills for structured option UI
