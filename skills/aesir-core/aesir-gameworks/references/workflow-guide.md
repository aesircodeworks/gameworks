...
# Aesir Gameworks -- Complete Workflow Guide
...
>
> The pipeline has 7 phases. Each phase has a formal gate (`/aesir-gate-check`)
> that must pass before you advance. The authoritative phase sequence is
> defined in `aesir-gameworks references/workflow-catalog.yaml` and read by `/aesir-help`.
...
- **Path A** -- No idea yet: routes to `/aesir-brainstorm`
- **Path B** -- Vague idea: routes to `/aesir-brainstorm` with seed
- **Path C** -- Clear concept: routes to `/aesir-setup-engine` and `/aesir-map-systems`
- **Path D1** -- Existing project, few artifacts: normal flow
- **Path D2** -- Existing project, GDDs/ADRs exist: runs `/aesir-project-stage-detect`
  then `/aesir-adopt` for brownfield migration
...
### Step 1.1: Brainstorm With /aesir-brainstorm
...
**What /aesir-setup-engine does:**

- Populates `aesir-gameworks references/technical-preferences.md` with naming conventions,
...
`/aesir-design-system`, which guides you through creating its GDD section by section.
...
> **Tip:** `/aesir-design-system` emits a 📌 UX Flag for every system with UI
...
**Note on concept prototyping:** If you ran `/aesir-prototype` in Phase 1 (Concept),
...
- **PIVOT** → revise affected GDDs with `/aesir-design-system [mechanic]`, then re-run `/aesir-vertical-slice`
- **KILL** → return to `/aesir-brainstorm` with what you learned
...
`/aesir-create-epics` reads your GDDs, ADRs, and architecture to define epic scope —
one epic per architectural module. Then `/aesir-create-stories` breaks each epic into
...
Once stories exist, run `/aesir-dev-story [story-path]` to implement one — it routes
...
- Playtest report written (`/aesir-playtest-report`)

This is a **hard gate** -- `/aesir-gate-check` will auto-FAIL if a human has not
...
- Coordinated polish pass completed (`/aesir-team-polish`)
...
**Set your review intensity once during `/aesir-start`.** Saved to `production/review-mode.txt`.

| Mode | What runs | Best for |
|------|-----------|----------|
| `full` | All director gates at every step | New projects, learning the system |
| `lean` | Directors only at phase transitions (`/aesir-gate-check`) | Experienced devs |
...
time by editing `production/review-mode.txt` directly or re-running `/aesir-start`.

Full gate definitions and check pattern: `aesir-gameworks references/director-gates.md`
...
| `detect-gaps.sh` | Session start | Detects fresh projects (no engine, no concept) and suggests `/aesir-start` |
...
| `validate-skill-change.sh` | Skill file written | Advises running `/aesir-skill-test` after `(game-workspace)/skills/` changes |
...
machine-readable story tracker. Written by `/aesir-sprint-plan` (init) and
`/aesir-story-done` (status updates). Read by `/aesir-sprint-status`, `/aesir-help`, and
`/aesir-story-done` (next story). Eliminates fragile markdown scanning.
...
Phase gates are formal checkpoints. Run `/aesir-gate-check` with the transition name:
...
controls the status line and `/aesir-help` behavior.
...
|-------------|-------|------|
| Come up with a game idea | `/aesir-brainstorm` skill | -- |
...
|---------|---------|-------|
| `/aesir-start` | Guided onboarding, routes to right workflow | Any (first session) |
| `/aesir-help` | Context-aware "what do I do next?" | Any |
| `/aesir-project-stage-detect` | Full project audit to determine current phase | Any |
| `/aesir-setup-engine` | Configure engine, pin version, set preferences | 1 |
| `/aesir-adopt` | Brownfield audit and migration plan | Any (existing projects) |
| `/aesir-skill-improve` | Improve a skill via test-fix-retest loop | Any |

#### Game Design (6)

| Command | Purpose | Phase |
|---------|---------|-------|
| `/aesir-brainstorm` | Collaborative ideation with MDA analysis | 1 |
| `/aesir-map-systems` | Decompose concept into systems index | 1-2 |
| `/aesir-design-system` | Guided section-by-section GDD authoring | 2 |
| `/aesir-quick-design` | Lightweight spec for small changes | 2+ |
| `/aesir-review-all-gdds` | Cross-GDD consistency and design theory review | 2 |
| `/aesir-propagate-design-change` | Find ADRs/stories affected by GDD changes | 5 |

#### UX and Interface (2)

| Command | Purpose | Phase |
|---------|---------|-------|
| `/aesir-ux-design` | Author UX specs (screen/flow, HUD, patterns) | 4 |
| `/aesir-ux-review` | Validate UX specs for accessibility and GDD alignment | 4 |

#### Architecture (4)

| Command | Purpose | Phase |
|---------|---------|-------|
| `/aesir-create-architecture` | Master architecture document | 3 |
| `/aesir-architecture-decision` | Create or retrofit an ADR | 3 |
| `/aesir-architecture-review` | Validate all ADRs, dependency ordering | 3 |
| `/aesir-create-control-manifest` | Flat programmer rules from Accepted ADRs | 3 |

#### Stories and Sprints (8)

| Command | Purpose | Phase |
|---------|---------|-------|
| `/aesir-create-epics` | Translate GDDs + ADRs into epics (one per module) | 4 |
| `/aesir-create-stories` | Break a single epic into story files | 4 |
| `/aesir-dev-story` | Implement a story — routes to the correct programmer agent | 5 |
| `/aesir-sprint-plan` | Create or manage sprint plans | 4-5 |
| `/aesir-sprint-status` | Quick 30-line sprint snapshot | 5 |
| `/aesir-story-readiness` | Validate story is implementation-ready | 4-5 |
| `/aesir-story-done` | 8-phase story completion review | 5 |
| `/aesir-estimate` | Effort estimation with risk assessment | 4-5 |

#### Reviews and Analysis (13)

| Command | Purpose | Phase |
|---------|---------|-------|
| `/aesir-design-review` | Validate GDD against 8-section standard | 1-2 |
| `/aesir-code-review` | Architectural code review | 5+ |
| `/aesir-balance-check` | Game balance formula analysis | 5-6 |
| `/aesir-asset-audit` | Asset naming, format, size verification | 6 |
| `/aesir-asset-spec` | Per-asset visual specs and AI generation prompts | 5-6 |
| `/aesir-content-audit` | GDD-specified content vs. implemented | 5 |
| `/aesir-consistency-check` | Cross-GDD entity and formula inconsistency scan | 2+ |
| `/aesir-scope-check` | Scope creep detection | 5 |
| `/aesir-perf-profile` | Performance profiling workflow | 6 |
| `/aesir-tech-debt` | Tech debt scanning and prioritization | 6 |
| `/aesir-gate-check` | Formal phase gate with PASS/CONCERNS/FAIL | All transitions |
| `/aesir-reverse-document` | Generate design docs from existing code | Any |
| `/aesir-security-audit` | Security vulnerability audit (save, network, input) | 6-7 |

#### QA and Testing (9)

| Command | Purpose | Phase |
|---------|---------|-------|
| `/aesir-qa-plan` | Generate QA test plan for a sprint or feature | 5 |
| `/aesir-smoke-check` | Critical path smoke test gate before QA hand-off | 5-6 |
| `/aesir-soak-test` | Soak test protocol for extended play sessions | 6 |
| `/aesir-regression-suite` | Map test coverage, identify fixed bugs lacking regression tests | 5-6 |
| `/aesir-test-setup` | Scaffold test framework and CI/CD pipeline | 4 |
| `/aesir-test-helpers` | Generate engine-specific test helper libraries | 4-5 |
| `/aesir-test-evidence-review` | Quality review of test files and manual evidence | 5 |
| `/aesir-test-flakiness` | Detect non-deterministic tests from CI logs | 5-6 |
| `/aesir-skill-test` | Validate skill files for structural and behavioral correctness | Any |

#### Production Management (6)

| Command | Purpose | Phase |
|---------|---------|-------|
| `/aesir-milestone-review` | Milestone progress and go/no-go | 5 |
| `/aesir-retrospective` | Sprint retrospective analysis | 5 |
| `/aesir-bug-report` | Structured bug report creation | 5+ |
| `/aesir-bug-triage` | Re-evaluate open bugs for priority, severity, and owner | 5+ |
| `/aesir-playtest-report` | Structured playtest session report | 4-6 |
| `/aesir-onboard` | Onboard a new team member | Any |

#### Release (6)

| Command | Purpose | Phase |
|---------|---------|-------|
| `/aesir-release-checklist` | Pre-release validation | 7 |
| `/aesir-launch-checklist` | Full cross-department launch readiness | 7 |
| `/aesir-changelog` | Auto-generate internal changelog | 7 |
| `/aesir-patch-notes` | Player-facing patch notes | 7 |
| `/aesir-hotfix` | Emergency fix workflow | 7+ |
| `/aesir-day-one-patch` | Scoped patch for issues found after gold master | 7+ |

#### Creative (4)

| Command | Purpose | Phase |
|---------|---------|-------|
| `/aesir-prototype` | Concept prototype — validate core idea before GDDs | 1 |
| `/aesir-art-bible` | Guided Art Bible authoring — visual identity spec | 1-2 |
| `/aesir-vertical-slice` | Production-quality end-to-end build before Production | 4 |
| `/aesir-localize` | String extraction and validation | 6-7 |

#### Team Orchestration (9)

| Command | Purpose | Phase |
|---------|---------|-------|
| `/aesir-team-combat` | Combat feature: design through implementation | 5 |
| `/aesir-team-narrative` | Narrative content: structure through dialogue | 5 |
| `/aesir-team-ui` | UI feature: UX spec through polished implementation | 5 |
| `/aesir-team-level` | Level: layout through dressed encounters | 5 |
| `/aesir-team-audio` | Audio: direction through implemented events | 5-6 |
| `/aesir-team-polish` | Coordinated polish: perf + art + audio + QA | 6 |
| `/aesir-team-release` | Release coordination: build + QA + deployment | 7 |
| `/aesir-team-live-ops` | Live-ops planning: seasonal events, battle pass, retention | 7+ |
| `/aesir-team-qa` | Full QA cycle: strategy, execution, coverage, sign-off | 6-7 |
...
2. **Use team skills for cross-cutting features.** Do not try to manually
   coordinate 4 agents yourself -- let `/aesir-team-combat`, `/aesir-team-narrative`,
...
6. **Run /aesir-help when uncertain.** It reads your actual project state and tells
   you the single most important next step.

7. **Run `/aesir-design-review` before handing designs to programmers.** This
   catches incomplete specs early, saving rework.

8. **Run `/aesir-code-review` after every major feature.** Catch architectural
...
10. **Keep your sprint plans honest.** Use `/aesir-scope-check` regularly. Scope
...
12. **Use the story lifecycle religiously.** `/aesir-story-readiness` before pickup,
    `/aesir-story-done` after completion. This catches deviations early and keeps
    the pipeline honest.
