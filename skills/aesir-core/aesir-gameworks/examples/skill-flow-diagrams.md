...
## Skill Chain: /aesir-design-system in Detail
...
For projects with existing work (use `/aesir-start` option D or run directly):
...
|---------------|---------|
| Brand new, no idea | `/aesir-start` → `/aesir-brainstorm` |
| Have a concept, no engine | `/aesir-setup-engine` |
| Have concept + engine | `/aesir-map-systems` |
| Mid-systems design | `/aesir-design-system [next system]` or `/aesir-map-systems next` |
| All GDDs done | `/aesir-review-all-gdds` → `/aesir-gate-check` |
| In technical setup | `/aesir-create-architecture` → `/aesir-architecture-decision` |
| Starting UX design | `/aesir-ux-design screen [name]` or `/aesir-ux-design hud` |
| Scaffolding tests | `/aesir-test-setup` → `/aesir-test-helpers` |
| Have stories, ready to code | `/aesir-story-readiness [story]` → `/aesir-dev-story [story]` |
| Story done | `/aesir-story-done [story]` |
| Running QA for a sprint | `/aesir-qa-plan` → `/aesir-smoke-check` → `/aesir-regression-suite` |
| Bug backlog needs sorting | `/aesir-bug-triage` |
| Extended stability testing | `/aesir-soak-test` |
| Not sure | `/aesir-help` |
| Existing project | `/aesir-adopt` |
