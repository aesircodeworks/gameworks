...
# Example Session: Full Story Lifecycle — /aesir-story-readiness → Implement → /aesir-story-done

**Context:** Solo indie dev, mid-production sprint. The movement system GDD and architecture are complete. A story for movement's core implementation has been generated by `/aesir-create-stories`.
...
**Turns:** 13
**Outcome:** Story validated ready, implemented, verified complete with `/aesir-story-done`
...
**User:**
> /aesir-story-readiness production/stories/STORY-MOV-001.md

### Turn 2: Agent Validates the Story

**Agent (via /aesir-story-readiness):**
...
### Turn 10: User Tests Manually and Runs /aesir-story-done

**User:**
> I've tested it in the scene — everything works. /aesir-story-done production/stories/STORY-MOV-001.md

### Turn 11: Agent Runs Completion Review

**Agent (via /aesir-story-done):**
...
>
> Suggest running `/aesir-code-review src/gameplay/movement/` before marking complete. Not blocking.
>
> ---
...
>
> Run `/aesir-story-readiness` on whichever you pick up next.

---

## What This Example Demonstrates

- **Readiness gate catches real issues**: the roll direction ambiguity would have caused a late implementation decision without `/aesir-story-readiness`
...
- **Deferred criteria**: not every acceptance criterion can be verified at story close — `/aesir-story-done` tracks deferred items rather than blocking completion
...
- **sprint-status.yaml**: single source of truth for sprint state, updated by `/aesir-story-done` and read by `/aesir-sprint-status`
