...
# Example Session: /aesir-gate-check Phase Transition — Systems Design → Technical Setup
...
- Engine configured (Godot 4.6, `/aesir-setup-engine` complete)
...
> | `/aesir-design-review` per GDD | Each GDD reviewed | ✅ (6/6 review comments found) |
...
>
> 1. **`/aesir-create-architecture`** — Author the master architecture document. Covers system decomposition, data flow, layer boundaries, and module responsibilities.
>
> 2. **`/aesir-architecture-decision` (one per significant technical choice)** — At minimum:
>    - Rendering approach (2D camera, viewport, etc.)
>    - Save/load strategy
>    - Audio bus architecture
>    - The "item stack size authority" issue from cross-review (crafting vs inventory)
>
> 3. **`/aesir-architecture-review`** — Validates all ADRs for completeness, dependency ordering, and GDD traceability. Produces the TR-registry.
>
> 4. **`/aesir-create-control-manifest`** — Converts Accepted ADRs into a flat programmer rules sheet. This is what implementation agents will consult during coding.
>
> 5. **`/aesir-gate-check`** — Technical Setup → Pre-Production gate. Requires: architecture doc, all ADRs Accepted, control manifest, and dependency ordering resolved.
>
> **Optional but recommended:**
> - `/aesir-tech-debt` scan if you have any prototype code you plan to carry forward
> - `/aesir-setup-engine` re-run if you haven't yet fetched Godot 4.6 API snapshots
>
> **Tip:** Start with `/aesir-architecture-decision` for the item stack size conflict. It's a quick win that closes an open question before it creates ambiguity in two ADRs later.
...
>
> Run `/aesir-architecture-decision` and it will guide you through the ADR format, including the Dependencies section that links this ADR back to the two GDDs.

## Turn 7: User Wraps Up

**User:**
> Got it. I'll start with /aesir-architecture-decision for this.
...
>
> Your phase gate is logged. Run `/aesir-help` at any time during Technical Setup to see your current progress against the required checklist.
...
- **Stage.txt is the authority**: the status line, `/aesir-help`, and all skills read from `production/stage.txt` — updating it here changes what every subsequent skill sees
