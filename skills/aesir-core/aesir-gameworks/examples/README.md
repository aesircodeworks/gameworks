...
- Ends with explicit handoff: "run `/aesir-design-review` before the next system"

**Learn:**
- How `/aesir-design-system` is different from asking an agent to "write a GDD"
...
**Type:** Full Workflow
**Skills:** `/aesir-story-readiness` → implementation → `/aesir-story-done`
**Duration:** ~50 minutes (13 turns)
**Complexity:** Medium

**Scenario:**
Dev picks up a story from the sprint backlog. `/aesir-story-readiness` catches a roll-direction ambiguity before any code is written. After implementation, `/aesir-story-done` verifies 9 acceptance criteria, identifies 2 deferred criteria (inventory not integrated yet), and closes the story with notes.

**Key Moments:**
- `/aesir-story-readiness` catches spec ambiguity in Turn 2 — resolved before implementation starts
...
**Learn:**
- Why `/aesir-story-readiness` prevents late-implementation ambiguity
...
**Type:** Phase Gate
**Skill:** `/aesir-gate-check`
**Duration:** ~20 minutes (7 turns)
**Complexity:** Low

**Scenario:**
Dev completes the Systems Design phase and runs `/aesir-gate-check` to advance. The gate finds all 6 MVP GDDs complete, cross-review passed with one low-severity concern. Gate passes, `stage.txt` updated, and the agent provides a specific ordered checklist for Technical Setup.
...
- stage.txt update changes what `/aesir-help`, `/aesir-sprint-status`, and all skills see going forward
...
### [Session: UX Pipeline — /aesir-ux-design → /aesir-ux-review → /aesir-team-ui](session-ux-pipeline.md)
**Type:** UX Design Pipeline
**Skills:** `/aesir-ux-design`, `/aesir-ux-review`, `/aesir-team-ui`
**Duration:** ~90 minutes (16 turns)
**Complexity:** Medium-High

**Scenario:**
Dev designs the HUD and inventory screen. `/aesir-ux-design` reads the player journey and GDDs to ground decisions in player emotional state. `/aesir-ux-review` catches a blocking accessibility gap (no keyboard alternative to drag-drop) and an advisory colorblind issue. After fixes, `/aesir-team-ui` accepts the handoff.

**Key Moments:**
- HUD philosophy choice (diegetic vs. persistent vs. tactical) grounded in survival genre conventions
- `/aesir-ux-review` distinguishes BLOCKING (stops handoff) vs. ADVISORY (can fix in visual pass)
- Accessibility caught before implementation, not during QA
- Keyboard alternative added in one turn; review re-runs and passes
- `/aesir-team-ui` checks for a passing `/aesir-ux-review` before starting visual design

**Learn:**
- How `/aesir-ux-design` uses player journey context to ground UI decisions
- What `/aesir-ux-review` actually checks (not just "does a spec exist?")
...
### [Session: Brownfield Onboarding with /aesir-adopt](session-adopt-brownfield.md)
...
**Scenario:**
Dev has 3 months of existing code and rough design notes but nothing in the right format. `/aesir-adopt` audits format compliance (not just file existence), classifies 4 gaps by severity, builds an ordered 7-step migration plan, and immediately fixes the BLOCKING gap (missing systems index) by inferring it from the codebase.
...
- Retrofit mode vs. new authoring: `/aesir-design-system retrofit` fills gaps without overwriting

**Learn:**
- The difference between `/aesir-adopt` and `/aesir-project-stage-detect`
...
> In practice, agents now use the `clarify` tool at decision points to
...
- **Running /aesir-design-system for the first time?** → Read session-design-system-skill.md
...
- **Agent Roster:** [aesir-gameworks references/agent-roster.md](../../aesir-gameworks references/agent-roster.md)
