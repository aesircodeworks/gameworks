...
# Example Session: /aesir-adopt — Brownfield Project Onboarding
...
## Turn 1: User Invokes /aesir-adopt
...
**Agent (technical-director, via /aesir-adopt, context: fork):**
...
> | Gap | Severity | Reason |
> |-----|----------|--------|
> | No systems index | BLOCKING | `/aesir-design-system`, `/aesir-create-stories`, `/aesir-gate-check` all require it |
> | GDD files not in template format | HIGH | Skills cannot reference TR-IDs from malformed GDDs |
> | No architecture docs | HIGH | Cannot run `/aesir-architecture-review` or `/aesir-create-control-manifest` |
...
> |---|------|-------|----------|-------|
> | 1 | Create `design/gdd/systems-index.md` | `/aesir-map-systems` or `/aesir-adopt` inline | BLOCKING | Unblocks all downstream skills |
> | 2 | Retrofit `design/inventory.md` → GDD format | `/aesir-design-system retrofit design/inventory.md` | HIGH | Adds 5 missing sections, keeps your existing content |
> | 3 | Retrofit `design/combat-notes.md` → GDD format | `/aesir-design-system retrofit design/combat-notes.md` | HIGH | Adds 6 missing sections |
> | 4 | Convert `design/crafting-ideas.md` → full GDD | `/aesir-design-system crafting` (new authoring, uses ideas as input) | MEDIUM | Not a retrofit — too informal to retrofit |
> | 5 | Create architecture ADRs from existing code | `/aesir-reverse-document` + `/aesir-architecture-decision` | HIGH | Captures decisions already made in code |
> | 6 | Create master architecture document | `/aesir-create-architecture` | HIGH | After ADRs |
> | 7 | Set up production tracking | `/aesir-sprint-plan new` | MEDIUM | After architecture phase |
>
...
>
> I can create one now by reading your existing code and design docs to infer what systems you've built. This is faster than running `/aesir-map-systems` from scratch — I'll populate it based on what already exists, then you confirm or adjust.
>
...
>
> - ✅ `/aesir-design-system [name]` — guided GDD authoring for any system
> - ✅ `/aesir-design-system retrofit [path]` — fill missing sections in existing docs
> - ✅ `/aesir-gate-check` — now has a systems index to validate
> - ✅ `/aesir-help` — can now show correct phase workflow
>
> **Recommended next steps (from adoption plan):**
>
> 1. Run `/aesir-design-system retrofit design/inventory.md` — your most complete existing doc, easiest win
> 2. Run `/aesir-design-system retrofit design/combat-notes.md`
> 3. Run `/aesir-design-system crafting` — author crafting GDD using your ideas file as input
>
> After those 3, you'll have 5 of 7 systems with complete GDDs. Then run `/aesir-review-all-gdds` to catch cross-system consistency issues before architecture work.
...
- **FORMAT audit, not existence audit**: `/aesir-adopt` doesn't just check whether files exist — it checks whether their internal structure matches what skills expect. A file named `gdd.md` with no template sections is flagged as a gap.
...
- **Retrofit vs. new authoring**: `/aesir-design-system retrofit` handles existing-but-incomplete docs; full `/aesir-design-system` handles net-new docs. The adoption plan distinguishes which applies to each file.
- **Context: fork**: `/aesir-adopt` runs in a forked context to avoid polluting the main session with large file reads across the whole codebase.
