...
description: Use when running the Aesir art-bible workflow. Guided, section-by-section
  ...
  production. Run after /aesir-brainstorm is approved and before /aesir-map-systems
...
> "No game concept found. Run `/aesir-brainstorm` first — the art bible is authored after the game concept is approved."
...
Read `aesir-gameworks references/technical-preferences.md` if it exists — extract performance budgets and engine for asset standard constraints.
...
Present the art-director's draft to the user. Use `clarify`:
...
Collect both. If they conflict (e.g., art-director wants elaborate diegetic UI but ux-designer flags it would reduce combat readability), surface the conflict explicitly with both positions. Do NOT silently resolve — use `clarify` to let the user decide.
...
- **`technical-artist`**: Engine-specific hard constraints — poly count budgets per asset category, texture memory limits, material slot counts, importer constraints, anything from the performance budgets in `aesir-gameworks references/technical-preferences.md`
...
After all sections are complete (or the scoped set from Phase 1 is complete), spawn `creative-director` via Task using gate **AD-ART-BIBLE** (`aesir-gameworks references/director-gates.md`).
...
- Does `aesir-gameworks references/technical-preferences.md` contain a configured engine (not `[TO BE CONFIGURED]`)? → setup-engine is done, skip that option
...
- Do GDDs exist (check above)? → include /aesir-consistency-check option
...
- `[_] Run /aesir-map-systems — decompose the concept into systems before writing GDDs` (skip if systems-index.md exists)
- `[_] Run /aesir-setup-engine — configure the engine (asset standards may need revisiting after engine is set)` (skip if engine configured)
- `[_] Run /aesir-design-system — start the first GDD` (skip if any GDDs exist)
- `[_] Run /aesir-review-all-gdds — cross-GDD consistency check (required before Technical Setup gate)` (skip if gdd-cross-review-*.md exists)
- `[_] Run /aesir-asset-spec — generate per-asset visual specs and AI generation prompts from approved GDDs` (include if GDDs exist)
- `[_] Run /aesir-consistency-check — scan existing GDDs against the art bible for visual direction conflicts` (include if GDDs exist)
- `[_] Run /aesir-create-architecture — author the master architecture document (next Technical Setup step)`
- `[_] Stop here`
...
> **Always include** `/aesir-create-architecture` and Stop here as options — these are always valid next steps once the art bible is complete.
...
- Run `/aesir-map-systems` to decompose the concept into game systems before authoring GDDs
- Run `/aesir-setup-engine` if the engine is not yet configured (asset standards may need revisiting after engine selection)
- Run `/aesir-design-system [first-system]` to start authoring per-system GDDs
- Run `/aesir-consistency-check` once GDDs exist to validate them against the art bible's visual rules
- Run `/aesir-create-architecture` to produce the master architecture document
