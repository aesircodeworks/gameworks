...
If a matching file is found, use `clarify`:
...
> "No sprint data found for [sprint/milestone]. Run `/aesir-sprint-status` to generate
> sprint data first, or provide the sprint details manually."

Then use `clarify` to present two options:
...
Run git log for the sprint period to understand what was actually committed and when. Use the terminal tool (which uses Git Bash on Windows — the `2>/dev/null` is bash syntax, not PowerShell):
...
If the user selects [A]: Proceed to invoke `/aesir-sprint-plan new`, passing the retrospective file path and a summary of the action items and velocity change so the sprint planner can reference them.

- If this was a milestone retrospective, run `/aesir-gate-check` to formally assess readiness for the next phase.
