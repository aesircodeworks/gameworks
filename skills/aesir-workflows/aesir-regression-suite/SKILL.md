...
- Before a release gate (`/aesir-gate-check polish` requires regression suite exists)
...
**Modes:**
- `/aesir-regression-suite update` — scan new bug fixes this sprint and check
  for regression test presence; add new tests to the suite manifest
- `/aesir-regression-suite audit` — full audit of all GDD critical paths vs.
  existing test coverage; flag paths with no regression test
- `/aesir-regression-suite report` — read-only status report (no writes); suitable
  for sprint reviews
...
- For each HIGH priority gap: "Consider creating the missing regression test
  before the next sprint. Run `/aesir-test-helpers` to scaffold the test file."
...
- If coverage drift detected: "Regression suite may be drifting. Consider
  running `/aesir-regression-suite audit` at the next sprint boundary."
