...
Uses `clarify` at each phase transition. Delegates all file writes to
sub-agents and sub-skills (`/aesir-ux-design`, `ui-programmer`). Produces a summary report
with verdict COMPLETE / BLOCKED and handoffs to `/aesir-ux-review`, `/aesir-code-review`,
`/aesir-team-polish`.
...
- [ ] Has a next-step handoff at the end (references `/aesir-ux-review`, `/aesir-code-review`, `/aesir-team-polish`)
...
- Engine UI specialist configured in `aesir-gameworks references/technical-preferences.md`

**Input:** `/aesir-team-ui inventory screen`
...
2. Phase 1b — `/aesir-ux-design inventory-screen` invoked (or ux-designer spawned directly); produces `design/ux/inventory-screen.md` using `ux-spec.md` template; `clarify` confirms spec before review
3. Phase 1c — `/aesir-ux-review design/ux/inventory-screen.md` invoked; returns APPROVED; gate passed, proceed to Phase 2
4. Phase 2 — art-director spawned; reviews full UX spec (not only wireframes); applies visual treatment; verifies color contrast; produces visual design spec with asset manifest; `clarify` confirms before Phase 3
...
- [ ] Next steps include `/aesir-ux-review`, `/aesir-code-review`, `/aesir-team-polish`
...
- `/aesir-ux-review` returns verdict NEEDS REVISION with specific concerns flagged (e.g., gamepad navigation flow incomplete, contrast ratio below minimum)

**Input:** `/aesir-team-ui inventory screen`

**Expected behavior:**
1. Phase 1a + 1b complete — UX spec produced
2. Phase 1c — `/aesir-ux-review design/ux/inventory-screen.md` returns NEEDS REVISION
3. Skill does NOT advance to Phase 2
4. `clarify` presented with the specific flagged concerns and options:
...
5. If user chooses (a): ux-designer revises spec, `/aesir-ux-review` re-run; loop continues until APPROVED or user overrides
...
- [ ] `clarify` presents the specific flagged concerns before offering options
...
**Input:** `/aesir-team-ui inventory screen` (resuming from Phase 3 complete)
...
- [ ] `clarify` shows all three review streams' results together before Phase 5 approval
...
**Input:** `/aesir-team-ui settings menu`
...
3. `clarify` presented with options:
   - (a) Run `/aesir-ux-design patterns` first to establish the pattern library, then continue
...
- [ ] `clarify` offers a "create pattern library first" option (referencing `/aesir-ux-design patterns`)
...
- [ ] Next steps present at end: `/aesir-ux-review`, `/aesir-code-review`, `/aesir-team-polish`

---

## Coverage Notes

- The HUD-specific path (`/aesir-ux-design hud` + `hud-design.md` template + visual budget check in Phase 5)
