...
# Skill Test Spec: /aesir-setup-engine

## Skill Summary

`/aesir-setup-engine` configures the project's engine, language, rendering backend,
...
(e.g., `/aesir-setup-engine godot`) to skip the engine-selection step. For each
...
- [ ] Has a next-step handoff (e.g., `/aesir-brainstorm` or `/aesir-start` depending on flow)

---

## Director Gate Checks

None. `/aesir-setup-engine` is a technical configuration skill. No director gates apply.
...
**Input:** `/aesir-setup-engine godot`
...
**Input:** `/aesir-setup-engine unity`
...
**Input:** `/aesir-setup-engine unreal`
