...
- **Full spec**: `/aesir-setup-engine godot 4.6` — engine and version provided
- **Engine only**: `/aesir-setup-engine unity` — engine provided, version will be looked up
- **No args**: `/aesir-setup-engine` — fully guided mode (engine recommendation + version)
- **Refresh**: `/aesir-setup-engine refresh` — update reference docs (see Section 10)
- **Upgrade**: `/aesir-setup-engine upgrade [old-version] [new-version]` — migrate to a new engine version (see Section 11)
...
- Read `design/gdd/game-concept.md` if it exists — extract genre, scope, platform
  targets, art style, team size, and any engine recommendation from `/aesir-brainstorm`
- If no concept exists, inform the user:
  > "No game concept found. Consider running `/aesir-brainstorm` first to discover what
...
**Question 2 — Target platform** (ask this second, always, via `clarify` — platform eliminates or heavily weights engines before any other factor):
...
5. Use `clarify` to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use `clarify` with concept-specific deep-dive topics. Always generate these options from the user's actual concept — do not use generic options. Always include at minimum:
...
After updating CLAUDE.md, create or update `aesir-gameworks references/technical-preferences.md` with
...
### Remaining Sections
- **Performance Budgets**: Use `clarify`:
...
Run `/aesir-setup-engine refresh` to populate full reference docs at any time.
```
...
If invoked as `/aesir-setup-engine refresh`:
...
If invoked as `/aesir-setup-engine upgrade [old-version] [new-version]`:
...
Use web_search and web_extract to locate the official migration guide between
...
2. Run /aesir-setup-engine refresh after upgrading the actual engine binary to
   verify no new deprecations were missed
3. Run /aesir-architecture-review — the engine upgrade may invalidate ADRs that
   reference specific APIs or engine capabilities
4. If any ADRs are invalidated, run /aesir-propagate-design-change to update
   downstream stories
```
...
2. [If from /aesir-brainstorm] Run /aesir-map-systems to decompose your concept into individual systems
3. [If from /aesir-brainstorm] Run /aesir-design-system to author per-system GDDs (guided, section-by-section)
4. [If from /aesir-brainstorm] Run /aesir-prototype [core-mechanic] to validate the core idea before writing GDDs
5. [If fresh start] Run /aesir-brainstorm to discover your game concept
6. Create your first milestone: /aesir-sprint-plan new
```
