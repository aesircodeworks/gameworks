...
- If the fix changes a tuning knob defined in a GDD or referenced by an ADR, remind the user:
  > "This value is defined in a design document. Run `/aesir-propagate-design-change [path]` on the affected GDD to find downstream impacts before committing."
...
- Confirm the file was written, then end with: "Re-run `/aesir-balance-check` after fixes to verify."

If [C]:
- Summarize open issues and end with: "Re-run `/aesir-balance-check` after fixes to verify."
