...
Present the qa-lead's full strategy to the user, then use `clarify`:
...
options:
  ...
  - "Smoke check failed — fix issues and re-run /aesir-team-qa"
  - "Cancel — resolve blockers first"
```

If smoke check **FAIL**: do not proceed to Phase 3. Surface the failures from the smoke check report and stop. The user must fix them, re-run `/aesir-smoke-check sprint`, and then re-run `/aesir-team-qa`.
If smoke check **UNKNOWN**: surface a warning — "No smoke check report found. Recommend running `/aesir-smoke-check sprint` before QA. Proceeding with caution."
...
Use `clarify` per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use `clarify` for each:
...
- APPROVED: "Build is ready for the next phase. Run `/aesir-gate-check` to validate advancement."
- APPROVED WITH CONDITIONS: "Resolve conditions before advancing. S3/S4 bugs may be deferred to polish."
- NOT APPROVED: "Resolve S1/S2 bugs and re-run `/aesir-team-qa` or targeted manual QA before advancing."
