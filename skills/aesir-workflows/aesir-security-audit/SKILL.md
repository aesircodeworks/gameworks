...
Read `aesir-gameworks references/technical-preferences.md` to determine:
...
**Audited by**: security-engineer via /aesir-security-audit
...
Run `/aesir-security-audit` again after remediating any CRITICAL or HIGH findings.
...
After remediating findings, re-run: `/aesir-security-audit quick` to confirm CRITICAL/HIGH items are resolved before running `/aesir-gate-check release`.

If CRITICAL findings exist:
> "⛔ CRITICAL security findings must be resolved before any public release. Do not proceed to `/aesir-launch-checklist` until these are addressed."

If no CRITICAL/HIGH findings:
> "✅ No blocking security findings. Report written to `production/security/`. Include this path when running `/aesir-gate-check release`."
