...
3. If no version is discoverable: use `clarify` to ask "What version number should be released? (e.g., v1.0.0)" and wait for user input before proceeding. Do NOT default to a hardcoded version string.
...
- Generate the release checklist using `/aesir-release-checklist`
...
- Generate changelog using `/aesir-changelog`
- Deploy to staging for final smoke test
- Deploy to production
- Human team action: Monitor dashboards and error rates for 48 hours post-release. Schedule a follow-up retrospective using `/aesir-retrospective` at the 48-hour mark.

Delegate to **community-manager** (in parallel with deployment):
- Finalize patch notes using `/aesir-patch-notes [version]`
...
- Run `/aesir-retrospective` if significant issues occurred during the release.
