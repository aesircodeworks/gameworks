...
# Skill Test Spec: /aesir-team-release
...
- [ ] Has a next-step handoff referencing post-release monitoring, `/aesir-retrospective`, and `production/stage.txt`
- [ ] Uses `clarify` at phase transitions requiring user approval before proceeding
...
**Input:** `/aesir-team-release v1.0.0`
...
2. Phase 2: Spawns `release-manager` via Task; cuts release branch from agreed commit; bumps version numbers; invokes `/aesir-release-checklist`; freezes branch; output: branch name and checklist; clarify: user approves before Phase 3
...
6. Phase 6: Spawns `release-manager` + `devops-engineer` (parallel); tags release in version control; invokes `/aesir-changelog`; deploys to staging; smoke test passes; deploys to production; simultaneously spawns `community-manager` to finalize patch notes via `/aesir-patch-notes v1.0.0` and prepare launch announcement
...
- [ ] `/aesir-changelog` is invoked by release-manager in Phase 6 (not written directly)
- [ ] `/aesir-patch-notes v1.0.0` is invoked by community-manager in Phase 6
...
**Input:** `/aesir-team-release v0.9.0`
...
- [ ] clarify offers the user resolution options (fix and re-run / defer / override with rationale)
...
**Input:** `/aesir-team-release v2.1.0`
...
**Input:** `/aesir-team-release v1.2.0`
...
4. clarify: options presented — (a) Fix translations and re-run Phase 4, (b) Remove French locale from this release, (c) Ship as-is with a known issues note
...
- [ ] clarify is used to offer the user resolution choices — the skill does not auto-waive
...
**Input:** `/aesir-team-release` (no argument)
...
3. Confirms with clarify before beginning Phase 1 proper: "Releasing v1.1.0. Is this correct?"
4. Proceeds as if `/aesir-team-release v1.1.0` was the input

**Expected behavior (variant B):**
1. Phase 1: No argument provided; reads available state files — no version discoverable
2. Uses clarify: "What version number should be released? (e.g., v1.0.0)"
...
- [ ] Inferred version is confirmed with the user via clarify before proceeding (variant A)
- [ ] When no version is discoverable, clarify is used — skill does not guess (variant B)
...
- [ ] Next steps always include 48-hour post-release monitoring, `/aesir-retrospective` recommendation, and `production/stage.txt` update to `Live`
...
- The "devops-engineer build fails" path is not separately tested — it would surface as a BLOCKED result in Phase 3 and follow the standard error recovery protocol (surface → assess → clarify options). This is validated structurally by the Static Assertions error recovery check.
...
- The "override NO-GO with documented rationale" path in Case 2 is referenced but not exhaustively tested — it is an escape hatch that the skill must support, and its existence is validated by the clarify options assertion in Case 2.
