---
name: aesir-create-control-manifest
...
**When to run:** After `/aesir-architecture-review` passes and ADRs are in Accepted
...
Spawn `technical-director` via Task using gate **TD-MANIFEST** (`aesir-gameworks references/director-gates.md`).
...
- **CONCERNS** → surface via `clarify` with options: `Revise flagged rules` / `Accept and proceed` / `Discuss further`
...
> **Status**: [Active — regenerate with `/aesir-create-control-manifest update` when ADRs change]

`Manifest Version` is the date this manifest was generated. Story files embed
this date when created. `/aesir-story-readiness` compares a story's embedded version
...
- If epics/stories don't exist yet: "Run `/aesir-create-epics layer: foundation` then `/aesir-create-stories [epic-slug]` — programmers
