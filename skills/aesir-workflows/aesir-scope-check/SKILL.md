...
- **CONCERNS** → offer to identify the 2–3 additions with best cut ratio. Reference `/aesir-sprint-plan update` to formally re-scope.
- **FAIL** → recommend escalating to producer. Reference `/aesir-sprint-plan update` for re-planning or `/aesir-estimate` to re-baseline timeline.

Always end with:
> "Run `/aesir-scope-check [name]` again after cuts are made to verify the verdict improves."
