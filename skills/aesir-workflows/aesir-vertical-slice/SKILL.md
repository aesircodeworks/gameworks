...
whether the core idea is worth designing, run `/aesir-prototype` (concept prototype) instead.
...
- Naming conventions from `aesir-gameworks references/technical-preferences.md`
...
Read `aesir-project-templates templates/vertical-slice-report.md` to get the report structure.
...
- `full` → spawn `creative-director` via Task using gate **CD-PLAYTEST**
  (`(game-workspace)/docs/director-gates.md`).
...
Recommended next steps:
- `/aesir-create-epics layer:foundation` — plan Foundation layer epics
- `/aesir-create-epics layer:core` — plan Core layer epics
- `/aesir-create-stories [epic-slug]` — break each epic into implementable stories
- `/aesir-sprint-plan` — plan the first sprint using velocity data from the slice
- `/aesir-gate-check pre-production` — formally advance the stage to Production

**Playtest note:** `/aesir-gate-check` will look for documented playtest evidence.
...
differently. When `/aesir-vertical-slice` is next run after a PIVOT, check the
`prototypes/` directory for a `PIVOT-NOTE.md` — use it to frame the new validation
question and inform scope decisions.

- Revise affected GDDs with `/aesir-design-system [mechanic]`
- Address architecture issues via `/aesir-architecture-decision`
- Then re-run `/aesir-vertical-slice` to validate the revised direction
...
- Return to `/aesir-brainstorm` with what you learned
- Or run `/aesir-prototype [new-concept]` to test a new direction cheaply first
