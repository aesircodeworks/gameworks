...
**Argument:** `$ARGUMENTS` (blank = ask user via clarify)
...
**Date**: [date]
**Generated by**: /aesir-qa-plan
**Scope**: [N stories across [N systems]]
**Engine**: [engine name from (game-workspace)/docs/technical-preferences.md, or "Not configured"]
...
*Smoke tests are verified by the developer via `/aesir-smoke-check`. Reference this
...
- [ ] Smoke check passes (run `/aesir-smoke-check sprint` before QA hand-off)
- [ ] No regressions introduced
- [ ] Code reviewed (via `/aesir-code-review` or documented peer review)
- [ ] Story file updated to `Status: Complete` (via `/aesir-story-done`)
````
...
then ask two questions together using `clarify`:
...
options:
  ...
  - "Also back-fill test case specs into each story file's ## QA Test Cases section (Recommended — enables /aesir-dev-story and /aesir-code-review traceability)"
```
...
- Once all sprint stories are implemented, run `/aesir-smoke-check sprint` to gate QA hand-off — not yet, only after implementation is complete
- For Logic/Integration stories, create the test files at the listed paths
  before marking stories done — `/aesir-story-done` checks for them"
...
- Use `clarify` for scope selection when no argument is provided.
