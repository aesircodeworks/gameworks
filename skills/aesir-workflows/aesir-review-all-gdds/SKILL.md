...
**This is distinct from `/aesir-design-review`**, which reviews one GDD for internal
...
- Before `/aesir-create-architecture` begins (architecture built on inconsistent GDDs
...
Run `/aesir-consistency-check` after this review to populate the registry."
...
> then re-run `/aesir-review-all-gdds`."
...
- The engine name and version from `aesir-gameworks references/technical-preferences.md` and `docs/engine-reference/[engine]/VERSION.md`
...
append to `production/session-state/active.md`:
    ...
    ## Session Extract — /aesir-review-all-gdds [date]
    - Verdict: [PASS / CONCERNS / FAIL]
    - GDDs reviewed: [N]
...
- Are any GDDs in the "Flagged for Revision" table? → offer /aesir-design-review option for each
- Read systems-index.md for the next system with Status: Not Started → offer /aesir-design-system option
- Is the verdict PASS or CONCERNS? → offer /aesir-gate-check or /aesir-create-architecture
...
- `[_] Run /aesir-design-review [flagged-gdd-path] — address flagged warnings` (one per flagged GDD, if any)
- `[_] Run /aesir-design-system [next-system] — next in design order` (always include, name the actual system)
- `[_] Run /aesir-create-architecture — begin architecture (verdict is PASS/CONCERNS)` (include if verdict is not FAIL)
- `[_] Run /aesir-gate-check — validate Systems Design phase gate` (include if verdict is PASS)
- `[_] Stop here`
