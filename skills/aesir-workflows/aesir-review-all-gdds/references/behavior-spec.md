...
# Skill Test Spec: /aesir-review-all-gdds

## Skill Summary

`/aesir-review-all-gdds` is an Opus-tier skill that performs a holistic cross-GDD review
...
- [ ] Next-step handoff to `/aesir-architecture-review` or `/aesir-create-architecture` is present
...
- [ ] Skill suggests running `/aesir-design-system system-B` to resolve the gap
...
3. Skill recommends running `/aesir-brainstorm` and `/aesir-design-system` before re-running
...
- [ ] Skill recommends the correct next action (`/aesir-brainstorm` or `/aesir-design-system`)
...
- [ ] Ends with appropriate handoff: MAJOR ISSUES → fix and re-run; MINOR ISSUES → may proceed with awareness; CONSISTENT → `/aesir-create-architecture`
