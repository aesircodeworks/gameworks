...
> **Explicit invocation only**: This skill should only run when the user explicitly requests it with `/aesir-hotfix`. Do not auto-invoke based on context matching.
...
If the check passes, use `clarify` before creating the branch:
...
Apply the verdict:
- **Smoke check sufficient** — run `/aesir-smoke-check` against the hotfix build. If PASS, proceed to Phase 6.
- **Targeted QA pass required** — run `/aesir-team-qa [affected-system]` scoped to the changed system only. If QA returns APPROVED or APPROVED WITH CONDITIONS, proceed to Phase 6.
- **Full QA required** — S1 fixes that touch core systems may require a full `/aesir-team-qa sprint`. This delays deployment but prevents a bad patch.
...
Next: /aesir-bug-report verify [BUG-ID] after deploy to confirm resolution
```
...
After deploying, run `/aesir-bug-report verify [BUG-ID]` to confirm the fix resolved the issue in the deployed build.

If VERIFIED FIXED: run `/aesir-bug-report close [BUG-ID]` to formally close it.
If STILL PRESENT: the hotfix failed — immediately re-open, assess rollback, and escalate.

Schedule a post-incident review within 48 hours using `/aesir-retrospective hotfix`.

Use `clarify`:
- Prompt: "Hotfix complete. What's the next step?"
- Options:
  - `[A] Run /aesir-smoke-check to verify the fix`
  - `[B] Run /aesir-patch-notes to document this hotfix`
  - `[C] Stop here`
