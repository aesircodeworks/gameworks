...
# Skill Test Spec: /aesir-hotfix

## Skill Summary

`/aesir-hotfix` manages an emergency fix workflow: it creates a hotfix branch from
main, applies a targeted fix to the identified file(s), runs `/aesir-smoke-check` to
...
- [ ] Has a next-step handoff (e.g., `/aesir-bug-report` to document the issue, or version bump)
...
**Input:** `/aesir-hotfix` (user describes the crash and affected file)
...
5. Skill runs `/aesir-smoke-check` — PASS
...
- [ ] `/aesir-smoke-check` runs after the fix is applied
...
- `/aesir-smoke-check` returns FAIL: "Player health clamping regression detected"

**Input:** `/aesir-hotfix`

**Expected behavior:**
1. Skill applies the fix and runs `/aesir-smoke-check`
...
**Fixture:**
- User invokes `/aesir-hotfix` with a vague description: "something is broken on level 3"
- No repro steps provided

**Input:** `/aesir-hotfix` (vague description)
...
- [ ] Runs `/aesir-smoke-check` after applying the fix
