...
description: Use when running the Aesir create-epics workflow. Translate approved
  ...
  — run /aesir-create-stories [epic-slug] after each epic is created.
version: 1.0.0
...
**Next step after each epic:** `/aesir-create-stories [epic-slug]`

**When to run:** After `/aesir-create-control-manifest` and `/aesir-architecture-review` pass.
...
**Modes:**
- `/aesir-create-epics all` — process all systems in layer order
- `/aesir-create-epics layer: foundation` — Foundation layer only
- `/aesir-create-epics layer: core` — Core layer only
- `/aesir-create-epics layer: feature` — Feature layer only
- `/aesir-create-epics layer: presentation` — Presentation layer only
- `/aesir-create-epics [system-name]` — one specific system
...
> Run `/aesir-architecture-decision` first, or proceed with placeholders."
...
After all epics for the current layer are defined (Step 4 completed for all in-scope systems), and before writing any files, spawn `producer` via Task using gate **PR-EPIC** (`aesir-gameworks references/director-gates.md`).
...
> **Status**: Ready
> **Stories**: Not yet created — run `/aesir-create-stories [epic-slug]`
...
- All stories are implemented, reviewed, and closed via `/aesir-story-done`
...
Run `/aesir-create-stories [epic-slug]` to break this epic into implementable stories.
```
...
- **Foundation + Core complete**: These are required for the Pre-Production →
  Production gate. Run `/aesir-gate-check production` to check readiness.
- **Reminder**: Epics define scope. Stories define implementation steps. Run
  `/aesir-create-stories [epic-slug]` for each epic before developers can pick up work.
...
- **Verdict: COMPLETE** — [N] epic(s) written. Run `/aesir-create-stories [epic-slug]` per epic.
