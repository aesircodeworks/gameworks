...
# Skill Test Spec: /aesir-changelog

## Skill Summary

`/aesir-changelog` is a Haiku-tier skill that auto-generates a developer-facing
...
- [ ] Has a next-step handoff (e.g., run /aesir-patch-notes for player-facing version)
...
- The `/aesir-patch-notes` skill should be run after `/aesir-changelog` for player-facing
