...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `clarify`:
...
3. If no systems index exists, fail with:
   > "Usage: `/aesir-design-system <system-name>` — e.g., `/aesir-design-system movement`
   > Or to fill gaps in an existing GDD: `/aesir-design-system retrofit design/gdd/[system-name].md`
   > No systems index found. Run `/aesir-map-systems` first to map your systems and get the design order."
...
- **Game concept**: Read `design/gdd/game-concept.md` — fail if missing:
  > "No game concept found. Run `/aesir-brainstorm` first."
- **Systems index**: Read `design/gdd/systems-index.md` — fail if missing:
  > "No systems index found. Run `/aesir-map-systems` first to map your systems."
...
- Read `aesir-gameworks references/technical-preferences.md` to identify the engine and version
...
> `/aesir-setup-engine` before moving to architecture if you haven't already."
...
Use the template structure from `aesir-project-templates templates/game-design-document.md`:
...
> "Verdict: **BLOCKED** — skeleton creation declined. The design session cannot proceed without the skeleton file, as all subsequent phases use it as the base. Re-run `/aesir-design-system [system]` when ready to create the file."
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `clarify` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `clarify` to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   `clarify`. **NEVER use plain text. NEVER skip this step.**
...
7. **Write**: Use the patch tool to replace the placeholder with the approved content.
   **CRITICAL**: Always include the section heading in the `old_string` to ensure
   ...
   same placeholder and the patch tool requires a unique match. Use this pattern:
   ```
...
**Framing questions (ask BEFORE drafting)**: Use `clarify` with a multi-tab widget:
...
in `/aesir-architecture-decision`, not the GDD. The GDD describes behavior; the ADR
...
**Framing question (ask BEFORE drafting)**: Use `clarify`:
...
- Surface any disagreements between agents to the user via `clarify`
...
> "📌 **Asset Spec** — Visual/Audio requirements are defined. After the art bible is approved, run `/aesir-asset-spec system:[system-name]` to produce per-asset visual descriptions, dimensions, and generation prompts from this section."
...
> (Pre-Production), run `/aesir-ux-design` to create a UX spec for each screen or
...
Before finalizing the GDD, spawn `creative-director` via Task using gate **CD-GDD-ALIGN** (`aesir-gameworks references/director-gates.md`).
...
> **To validate this GDD, open a fresh Hermes Agent session and run:**
> `/aesir-design-review design/gdd/[system-name].md`
>
> **Never run `/aesir-design-review` in the same session as `/aesir-design-system`.** The reviewing
...
**NEVER offer to run `/aesir-design-review` inline.** Always direct the user to a fresh window.
...
- "What's next?"
  - Options:
    - "Run `/aesir-consistency-check` — verify this GDD's values don't conflict with existing GDDs (recommended before designing the next system)"
    - "Design next system ([next-in-order])" — if undesigned systems remain
    - "Fix review findings" — if design-review flagged issues
    - "Stop here for this session"
    - "Run `/aesir-gate-check`" — if enough MVP systems are designed
...
- The main session presents the agent's output to the user via `clarify`
- The user decides; the main session writes to file
...
2. **clarify** at every decision point (Explain -> Capture pattern):
...
> open a fresh Hermes Agent session and run `/aesir-design-system [system-name]` — it will
...
- Run `/aesir-design-review design/gdd/[system-name].md` in a **fresh session** to validate the completed GDD independently
- Run `/aesir-consistency-check` to verify this GDD's values don't conflict with other GDDs
- Run `/aesir-map-systems next` to move to the next highest-priority undesigned system
- Run `/aesir-gate-check pre-production` when all MVP GDDs are authored and reviewed
