...
to `/aesir-create-stories [epic-slug]` or `/aesir-quick-design` depending on the size of the gap.

### --summary mode

Print the Gap Table and Summary directly to conversation. Do not write a file.
End with: "Run `/aesir-content-audit` without `--summary` to write the full report."
...
- If any system is `NOT STARTED` and MVP-tagged → "Run `/aesir-design-system [name]` to
  add missing content counts to the GDD before implementation begins."
- If total gap is >50% → "Run `/aesir-sprint-plan` to allocate content work across upcoming sprints."
- If backlog stories are needed → "Run `/aesir-create-stories [epic-slug]` for each HIGH PRIORITY gap."
- If `--summary` was used → "Run `/aesir-content-audit` (no flag) to write the full report to `docs/`."
