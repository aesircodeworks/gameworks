...
- **B) Reduce feature scope** (run `/aesir-scope-check [feature]` to analyze trade-offs)
- **C) Accept the performance hit and defer to Polish phase** (log as known issue)
- **D) Escalate to technical-director for an architectural decision** (run `/aesir-architecture-decision`)
...
- If bottlenecks require architectural change: run `/aesir-architecture-decision`.
- If scope reduction is needed: run `/aesir-scope-check [feature]`.
- To schedule optimizations: run `/aesir-sprint-plan update`.
