...
description: Use when running the Aesir create-stories workflow. Break a single epic
  ...
  criteria, story type, and test evidence path. Run after /aesir-create-epics for
  each epic.
version: 1.0.0
...
**Previous step:** `/aesir-create-epics [system]`
**Next step after stories exist:** `/aesir-story-readiness [story-path]` then `/aesir-dev-story [story-path]`
...
in this skill — apply the check pattern from `aesir-gameworks references/director-gates.md`
before every gate invocation.

- `/aesir-create-stories [epic-slug]` — e.g. `/aesir-create-stories combat`
- `/aesir-create-stories production/epics/combat/EPIC.md` — full path also accepted
...
> Check the filename in the epic's Governing ADRs list, or run `/aesir-architecture-decision`
...
The type determines what test evidence is required before `/aesir-story-done` can close the story.
...
- **Governing ADR**: which ADR governs how to implement this?
  ...
  - `Status: Proposed` → set story `Status: Blocked` with note: "BLOCKED: ADR-NNNN is Proposed — run `/aesir-architecture-decision` to advance it"
...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `qa-lead` via Task using gate **QL-STORY-READY** (`aesir-gameworks references/director-gates.md`).
...
- If "Skip": leave `## QA Test Cases` with a placeholder: `*Test cases not yet defined — run /aesir-qa-plan to generate them.*`
...
> **Last Updated**: [set by /aesir-dev-story when implementation begins]
...
- Is this the last epic? If so, include `/aesir-sprint-plan` as an option.

Widget:
- Prompt: "[N] stories written to `production/epics/[epic-slug]/`. What next?"
- Options (include all that apply):
  - `[A] Start implementing — run /aesir-story-readiness [first-story-path]` (Recommended)
  - `[B] Create stories for [next-epic-slug] — run /aesir-create-stories [slug]` (only if other epics have no stories yet)
  - `[C] Plan the sprint — run /aesir-sprint-plan new` (only if all epics have stories)
...
- **Verdict: COMPLETE** — [N] stories written to `production/epics/[epic-slug]/`. Run `/aesir-story-readiness` → `/aesir-dev-story` to begin implementation.
