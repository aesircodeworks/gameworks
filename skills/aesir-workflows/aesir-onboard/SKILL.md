...
Read the relevant agent definition from `(game-workspace)/agents/` if a specific role is specified.
...
- Run `/aesir-sprint-status` to show the new contributor current progress.
- Run `/aesir-help` if the contributor needs guidance on what to work on next.
