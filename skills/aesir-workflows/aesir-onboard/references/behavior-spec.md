...
# Skill Test Spec: /aesir-onboard

## Skill Summary

`/aesir-onboard` generates a contextual project onboarding summary tailored for a new
...
The skill optionally accepts a role argument (e.g., `/aesir-onboard artist`) to tailor
...
None. `/aesir-onboard` is a read-only orientation skill. No director gates apply.
...
4. Next-step suggestions are appropriate for Production stage (e.g., `/aesir-sprint-status`,
   `/aesir-dev-story`)
...
### Case 2: Fresh Project — No engine, no sprint, suggests /aesir-start
...
3. Output explains the onboarding workflow: `/aesir-start` → `/aesir-setup-engine` → `/aesir-brainstorm`
4. Skill suggests running `/aesir-start` as the immediate next step
5. Verdict is ONBOARDING COMPLETE (informational, not a failure)

**Assertions:**
- [ ] Output explicitly mentions the project is not yet configured
- [ ] `/aesir-start` is recommended as the next step
...
3. Skill provides remediation: "Run `/aesir-start` to initialize the project configuration"
4. No partial summary is generated

**Assertions:**
- [ ] Error message clearly identifies the missing file as CLAUDE.md
- [ ] Remediation step (`/aesir-start`) is explicitly named
