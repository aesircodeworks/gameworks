...
# Skill Test Spec: /aesir-adopt

## Skill Summary

`/aesir-adopt` audits an existing project's artifacts — GDDs, ADRs, stories, infrastructure
...
after explicit user approval via `clarify`.

This skill is distinct from `/aesir-project-stage-detect` (which checks what exists).
`/aesir-adopt` checks whether what exists will actually work with the template's skills.
...
- [ ] Contains "May I write" or `clarify` language before writing the adoption plan
...
None. `/aesir-adopt` is a brownfield audit utility. No director gates apply.
...
4. Phase 4 builds ordered migration plan:
   - Step 1 (BLOCKING): Add `## Status` to `adr-0001.md` — command: `/aesir-architecture-decision retrofit`
   - Step 2 (HIGH): Run `/aesir-architecture-review` to bootstrap tr-registry.yaml
   - Step 3 (HIGH): Add Acceptance Criteria to `combat.md` — command: `/aesir-design-system retrofit`
...
6. `clarify` to write plan; writes after approval
...
### Case 4: No Artifacts Found — Fresh project, guidance to run /aesir-start
...
3. Uses `clarify`:
   ...
   - Options: "Run `/aesir-start`", "My artifacts are in a non-standard location", "Cancel"
4. Skill stops — does not proceed to audit regardless of user selection

**Assertions:**
- [ ] `clarify` is used (not a plain text message) when no artifacts are found
- [ ] `/aesir-start` is presented as a named option
...
4. No `/aesir-gate-check` is invoked during the skill run
...
- [ ] Uses `clarify` before writing the adoption plan file
