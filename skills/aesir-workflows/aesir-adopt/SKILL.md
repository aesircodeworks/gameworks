...
description: Use when running the Aesir adopt workflow. Brownfield onboarding — audits
  ...
  from /aesir-project-stage-detect (which checks what exists) — this checks whether
...
**This is not `/aesir-project-stage-detect`.**
`/aesir-project-stage-detect` answers: *what exists?*
`/aesir-adopt` answers: *will what exists actually work with the template's skills?*
...
- `aesir-gameworks references/technical-preferences.md` — engine configured?
...
Use the same heuristic as `/aesir-project-stage-detect`:
...
- Nothing → Fresh (not a brownfield project — suggest `/aesir-start`)

If the project appears fresh (no artifacts at all), use `clarify`:
- "This looks like a fresh project — no existing artifacts found. `/aesir-adopt` is for
  projects with work to migrate. What would you like to do?"
  - "Run `/aesir-start` — begin guided first-time onboarding"
...
|---|---|
| `## Status` | **BLOCKING** — `/aesir-story-readiness` ADR status check silently passes everything |
| `## ADR Dependencies` | HIGH — dependency ordering in `/aesir-architecture-review` breaks |
...
1. **Parenthetical status values** — Grep for any Status cell containing
   ...
   These break exact-string matching in `/aesir-gate-check`, `/aesir-create-stories`,
   and `/aesir-architecture-review`. **BLOCKING.**
...
| Sprint status | `production/sprint-status.yaml` | MEDIUM — `/aesir-sprint-status` falls back to markdown |
...
Read `aesir-gameworks references/technical-preferences.md`. Check each field for `[TO BE CONFIGURED]`:
...
`/aesir-architecture-decision retrofit docs/architecture/adr-[NNNN]-[slug].md`
...
`/aesir-design-system retrofit design/gdd/[filename].md`
...
2. Run `/aesir-architecture-review` → bootstraps `tr-registry.yaml`
3. Run `/aesir-create-control-manifest` → creates manifest with version stamp
4. Run `/aesir-sprint-plan update` → creates `sprint-status.yaml`
5. Run `/aesir-gate-check [phase]` → writes `stage.txt` authoritatively
...
Gap counts:
  ...
  HIGH:     [N] — unsafe to run /aesir-create-stories or /aesir-story-readiness
...
Re-run `/aesir-adopt` anytime to check remaining gaps.
...
Run `/aesir-architecture-review` — even if ADRs already exist, this run bootstraps
...
Run `/aesir-gate-check [current-phase]`
**Time**: 5 min
...
Run `/aesir-adopt` again after completing Step 3 to verify all blocking and high gaps
...
- **Options**:
  ...
  - `Lean (recommended)` — Directors only at phase gate transitions (/aesir-gate-check). Skips per-skill reviews. Balanced for solo devs and small teams.
...
- "The most urgent fix is `systems-index.md` — [N] rows have parenthetical status
  values (e.g. `Needs Revision (see notes)`) that break /aesir-gate-check,
  /aesir-create-stories, and /aesir-architecture-review right now. I can fix these in-place."
...
- "The most urgent fix is adding `## Status` to [N] ADR(s): [list filenames].
  Without it, /aesir-story-readiness silently passes all ADR checks. Start with
...
- "The most urgent gap is missing Acceptance Criteria in [N] GDD(s):
  [list filenames]. Without them, /aesir-create-stories can't generate stories.
...
- "No blocking gaps — this project is template-compatible. What next?"
  ...
  - "Run /aesir-project-stage-detect for a broader health check"
  - "Done — I'll work through the plan at my own pace"

> **Adoption plan saved to `docs/adoption-plan-[date].md`.** Re-run `/aesir-adopt` at any time to re-check remaining gaps as you complete them.
