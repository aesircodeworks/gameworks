...
- **No argument**: `/aesir-map-systems` — Run the full decomposition workflow (Phases 1-5)
  to create or update the systems index.
- **`next`**: `/aesir-map-systems next` — Pick the highest-priority undesigned system
  from the index and hand off to `/aesir-design-system` (Phase 6).
...
- Read `design/gdd/game-concept.md` — **fail with a clear message if missing**:
  > "No game concept found at `design/gdd/game-concept.md`. Run `/aesir-brainstorm` first
...
Then use `clarify` to capture feedback:
...
Use `clarify` to ask: "Does this dependency ordering look right? Any
...
**After dependency mapping is approved, spawn `technical-director` via Task using gate TD-SYSTEM-BOUNDARY (`aesir-gameworks references/director-gates.md`) before proceeding to priority assignment.**
...
Use `clarify` to ask: "Do these priority assignments match your vision?
...
**After priorities are approved, spawn `producer` via Task using gate PR-SCOPE (`aesir-gameworks references/director-gates.md`) before writing the index.**
...
Using the template at `aesir-project-templates templates/systems-index.md`, populate the
...
**After the systems index is written, spawn `creative-director` via Task using gate CD-SYSTEMS (`aesir-gameworks references/director-gates.md`).**
...
## Phase 6: Design Individual Systems (Handoff to /aesir-design-system)

This phase is entered when:
- The user says "yes" to designing systems after creating the index
- The user invokes `/aesir-map-systems [system-name]`
- The user invokes `/aesir-map-systems next`
...
### Step 6b: Hand Off to /aesir-design-system

Once a system is selected, invoke the `/aesir-design-system [system-name]` skill.

The `/aesir-design-system` skill handles the full GDD authoring process:
...
- Runs `/aesir-design-review` when complete
- Updates the systems index

**Do not duplicate the /aesir-design-system workflow here.** This skill owns the systems
*index*; `/aesir-design-system` owns individual system *GDDs*.

### Step 6c: Loop or Stop

After `/aesir-design-system` completes, use `clarify`:
...
- "Systems index is written. What would you like to do next?"
  - [A] Start designing GDDs — run `/aesir-design-system [first-system-in-order]`
  - [B] Run `/aesir-gate-check systems-design` — triggers the CD-SYSTEMS and TD-SYSTEM-BOUNDARY gates automatically for a formal director sign-off on the system set
  - [C] Stop here for this session

**The gate-check option ([B]) is worth highlighting**: running `/aesir-gate-check systems-design` triggers both the CD-SYSTEMS and TD-SYSTEM-BOUNDARY gates, catching scope issues, missing systems, and boundary problems before they're locked in across many documents. It is optional but recommended for new projects.

After any individual GDD is completed:
- "Run `/aesir-design-review design/gdd/[system].md` in a fresh session to validate quality"
- "Run `/aesir-gate-check systems-design` when all MVP GDDs are complete"
...
2. **clarify** at every decision point (Explain -> Capture pattern):
   ...
   - Phase 6: "Start designing, pick different, or stop?" then hand off to `/aesir-design-system`
...
5. **Handoff**: Individual GDD authoring is owned by `/aesir-design-system`, which handles
...
> `design/gdd/systems-index.md`. Open a fresh Hermes Agent session to continue
> designing individual GDDs — run `/aesir-map-systems next` to pick up where you left off.

---

## Recommended Next Steps

- Run `/aesir-design-system [first-system-in-order]` to author the first GDD (use design order from the index)
- Run `/aesir-map-systems next` to always pick the highest-priority undesigned system automatically
- Run `/aesir-design-review design/gdd/[system].md` in a fresh session after each GDD is authored
