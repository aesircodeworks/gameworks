...
# Skill Test Spec: /aesir-map-systems

## Skill Summary

`/aesir-map-systems` decomposes a game concept into a systems index. It reads the
...
- [ ] Has a next-step handoff at the end (`/aesir-design-system`)
...
2. File not found
3. Skill outputs: "No game concept found. Run `/aesir-brainstorm` to create one, then return to `/aesir-map-systems`."
4. Skill exits without creating systems-index.md

**Assertions:**
- [ ] Skill outputs a clear error naming the missing file path
- [ ] Skill recommends `/aesir-brainstorm` as the next action
...
- [ ] Ends with next-step handoff: `/aesir-design-system [next-system]`
...
- The `next` argument mode (handing off the highest-priority undesigned system to
  `/aesir-design-system`) is not tested here — it is a post-index-creation convenience.
