...
> Usage: `/aesir-team-audio [feature or area]` — specify the feature or area to design audio for (e.g., `combat`, `main menu`, `forest biome`, `boss encounter`). Do not use `clarify` here; output the guidance directly.
...
Spawn the **primary engine specialist** in parallel (from `aesir-gameworks references/technical-preferences.md` Engine Specialists) to validate the integration approach:
...
- Use `/aesir-dev-story` to implement the audio manager and event system once the design is approved.
- Run `/aesir-asset-audit` after audio assets are created to verify naming and format compliance.
