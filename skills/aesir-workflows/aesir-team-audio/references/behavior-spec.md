...
# Skill Test Spec: /aesir-team-audio
...
`clarify` at each step transition. Verdict is COMPLETE when the audio
...
- [ ] Has a next-step handoff at the end (references `/aesir-dev-story`, `/aesir-asset-audit`)
...
- Engine is configured in `aesir-gameworks references/technical-preferences.md`
- No accessibility gaps exist in the planned audio event list

**Input:** `/aesir-team-audio combat`
...
3. `clarify` presents audio direction; user approves before Step 2 begins
...
5. `clarify` presents SFX spec and accessibility requirements; user approves before Step 3 begins
...
**Input:** `/aesir-team-audio stealth` (Step 2 scenario)
...
3. Orchestrator surfaces the concern immediately in conversation before presenting `clarify`
4. `clarify` presents the accessibility concern as a BLOCKING issue with options:
...
- [ ] `clarify` surfaces the gap before Step 3 is launched
...
**Input:** `/aesir-team-audio` (no argument)

**Expected behavior:**
1. Skill detects no argument is provided
2. Outputs usage guidance: e.g., "Usage: `/aesir-team-audio [feature or area]` — specify the feature or area to design audio for (e.g., `combat`, `main menu`, `forest biome`, `boss encounter`)"
...
**Input:** `/aesir-team-audio main menu`
...
**Fixture:**
- Engine is NOT configured in `aesir-gameworks references/technical-preferences.md` (shows `[TO BE CONFIGURED]`)
- GDD for the target feature exists
- Sound bible may or may not exist

**Input:** `/aesir-team-audio boss encounter`

**Expected behavior:**
1. Context gathering: orchestrator reads `aesir-gameworks references/technical-preferences.md` and detects no engine is configured
...
- [ ] Next Steps handoff references `/aesir-dev-story` and `/aesir-asset-audit`
