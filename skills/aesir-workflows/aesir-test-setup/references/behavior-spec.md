...
# Skill Test Spec: /aesir-test-setup

## Skill Summary

`/aesir-test-setup` scaffolds the test framework for the project based on the
...
- [ ] Has a next-step handoff (e.g., `/aesir-test-helpers` to generate helper utilities)

---

## Director Gate Checks

None. `/aesir-test-setup` is a scaffolding utility. No director gates apply.
...
### Case 4: No Engine Configured — Redirects to /aesir-setup-engine
...
3. Skill suggests running `/aesir-setup-engine` first
4. No directories or files are created

**Assertions:**
- [ ] Error message explicitly states engine is not configured
- [ ] `/aesir-setup-engine` is suggested as the next step
