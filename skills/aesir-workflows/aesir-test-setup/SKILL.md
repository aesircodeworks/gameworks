...
1. **Read engine config**:
   - Read `aesir-gameworks references/technical-preferences.md` and extract the `Engine:` value.
   - If engine is not configured (`[TO BE CONFIGURED]`), stop:
     "Engine not configured. Run `/aesir-setup-engine` first, then re-run `/aesir-test-setup`."
...
3. **Report findings**:
   ...
   - If everything already exists AND `force` argument was not passed:
     "Test infrastructure appears to be in place. Re-run with `/aesir-test-setup force`
...
tests/
  ...
  smoke/          # Critical path test list for /aesir-smoke-check gate
...
```gdscript
# GdUnit4 test runner — invoked by CI and /aesir-smoke-check
...
**Run via**: `/aesir-smoke-check` (which reads this file)
...
3. Run `/aesir-qa-plan sprint` before your first sprint to classify stories and set
   test evidence requirements
4. `/aesir-smoke-check` before every QA hand-off

Gate note: /aesir-gate-check Technical Setup → Pre-Production now requires:
- tests/ directory with unit/ and integration/ subdirectories
- .github/workflows/tests.yml
- At least one example test file
Run /aesir-test-setup and write one example test before advancing.
...
- **Engine detection is non-negotiable** — if the engine is not configured,
  stop and redirect to `/aesir-setup-engine`. Do not guess.
