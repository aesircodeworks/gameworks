...
> **Explicit invocation only**: This skill should only run when the user explicitly requests it with `/aesir-launch-checklist`. Do not auto-invoke based on context matching.
...
- Run `/aesir-gate-check` to get a formal PASS/CONCERNS/FAIL verdict before launch.
- Coordinate sign-offs via `/aesir-team-release`.
