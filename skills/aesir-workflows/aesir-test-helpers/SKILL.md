...
**When to run:**
- After `/aesir-test-setup` scaffolds the framework (first time)
...
**Modes:**
- `/aesir-test-helpers [system-name]` — generate helpers for a specific system
  (e.g., `/aesir-test-helpers combat`)
- `/aesir-test-helpers all` — generate helpers for all systems with test files
- `/aesir-test-helpers scaffold` — generate only the base helper library (no
...
Read `aesir-gameworks references/technical-preferences.md` and extract:
- `Engine:` value
- `Language:` value
- `Framework:` from the Testing section

If engine is not configured: "Engine not configured. Run `/aesir-setup-engine` first."
...
## Generated by /aesir-test-helpers combat on [date].
...
- Run `/aesir-test-setup` if the test framework has not been scaffolded yet.
- Use `/aesir-dev-story` to implement stories — helpers reduce boilerplate in new test files.
- Run `/aesir-skill-test` to validate other skills that may need helper coverage.
