...
gap analysis, use `/aesir-project-stage-detect`.

---

## Step 1: Read the Catalog

Read `aesir-gameworks references/workflow-catalog.yaml`. This is the authoritative list of all
...
---
Approaching **[next phase]** gate → run `/aesir-gate-check` when ready.
```
...
- If all required steps in the current phase are complete (or nearly complete),
  add: "You're close to the **[Current] → [Next]** gate. Run `/aesir-gate-check` when ready."
...
---
Need more detail?
- `/aesir-project-stage-detect` — full gap analysis with all missing artifacts listed
- `/aesir-gate-check` — formal readiness check for your next phase
- `/aesir-start` — re-orient from scratch
```
