...
# Skill Test Spec: /aesir-help

## Skill Summary

`/aesir-help` analyzes what has been done and what comes next in the project workflow.
...
guidance summary. The skill optionally accepts a context query (e.g., `/aesir-help testing`)
...
None. `/aesir-help` is a read-only navigation skill. No director gates apply.
...
3. Skill outputs: current stage, sprint summary, and 3 suggested next skills
   (e.g., `/aesir-sprint-status`, `/aesir-dev-story`, `/aesir-story-done`)
...
3. Suggested skills are: `/aesir-brainstorm`, `/aesir-map-systems` (if concept exists)
...
- [ ] Suggestions do not include Production-stage skills (e.g., `/aesir-dev-story`)
- [ ] Verdict is HELP COMPLETE
...
4. Primary suggestion is `/aesir-start` to begin configuration

**Assertions:**
- [ ] Skill does not crash when stage.txt is absent
- [ ] Full workflow overview is shown when stage cannot be determined
- [ ] `/aesir-start` or `/aesir-project-stage-detect` is a top suggestion
- [ ] Verdict is HELP COMPLETE
...
**Input:** `/aesir-help testing`

**Expected behavior:**
1. Skill reads context query: "testing"
2. Skill surfaces skills relevant to testing: `/aesir-qa-plan`, `/aesir-smoke-check`,
   `/aesir-regression-suite`, `/aesir-test-setup`, `/aesir-test-evidence-review`
...
- [ ] General sprint skills (e.g., `/aesir-sprint-plan`) are not the primary suggestions
- [ ] Verdict is HELP COMPLETE
...
- The case where the active sprint is complete (all stories Done) is not
  separately tested; the skill would suggest `/aesir-sprint-plan` for the next sprint.
- The `/aesir-help` skill does not validate whether suggested skills are available —
  it assumes standard skill catalog availability.
- Stage detection fallback (when stage.txt is absent) delegates to the same
  logic as `/aesir-project-stage-detect` and is not re-tested here in detail.
