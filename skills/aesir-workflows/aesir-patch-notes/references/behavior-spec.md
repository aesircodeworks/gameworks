...
# Skill Test Spec: /aesir-patch-notes

## Skill Summary

`/aesir-patch-notes` is a Haiku-tier skill that generates player-facing patch notes
...
**Input:** `/aesir-patch-notes v0.4.0`
...
### Case 2: No Changelog Found — Directed to run /aesir-changelog first
...
2. Skill outputs: "No changelog found — run /aesir-changelog first to generate one"
3. No patch notes are generated; no file is written

**Assertions:**
- [ ] Skill does not crash when changelog is absent
- [ ] Output explicitly directs user to run `/aesir-changelog`
...
**Fixture:**
- `aesir-project-templates templates/patch-notes-template.md` exists with a structured header format
- `docs/CHANGELOG.md` exists with player-facing entries
