...
- Read the internal changelog at `production/releases/[version]/aesir-changelog.md` if it exists
...
**If no changelog data is available** (neither `production/releases/[version]/aesir-changelog.md`
nor a `docs/CHANGELOG.md` entry for this version exists, and git log is empty or unavailable):

> "No changelog data found for [version]. Run `/aesir-changelog [version]` first to generate the
> internal changelog, then re-run `/aesir-patch-notes [version]`."
...
1. Check `aesir-gameworks references/technical-preferences.md` for any "tone", "voice", or "style"
   fields or sections.
...
1. Glob for `docs/patch-notes-template.md` and `aesir-project-templates templates/patch-notes-template.md`.
...
if needed. Also write to `production/releases/[version]/aesir-patch-notes.md` as the
...
- Run `/aesir-release-checklist` to verify all other release gates are met before publishing.
