---
name: aesir-prototype
description: Use when running the Aesir prototype workflow. Concept prototype — validate
  the core idea is worth designing before writing GDDs. Run right after /aesir-brainstorm
  and /aesir-setup-engine. Routes to HTML, Engine, or Paper path based on game type.
...
**Default use** — run right after `/aesir-brainstorm` and `/aesir-setup-engine`, before writing
...
before committing to Production, run `/aesir-vertical-slice` instead.
...
Otherwise, use `clarify` to confirm intent before proceeding:
...
[answer]." Suggest next step: `/aesir-map-systems` or `/aesir-design-system [mechanic]`.
...
Read `CLAUDE.md` and `aesir-gameworks references/technical-preferences.md` for the engine and
language in use.
...
Assess which path best fits the hypothesis, then use `clarify` with your
...
Read `aesir-project-templates templates/prototype-report.md` to get the report structure.
...
1. `/aesir-design-review design/gdd/game-concept.md` — validate the concept doc against what the prototype revealed
2. `/aesir-gate-check` — confirm readiness to advance to Systems Design
3. `/aesir-art-bible` — define visual identity (optional but worth doing before GDDs)
4. `/aesir-map-systems` — decompose the concept into all game systems
5. `/aesir-design-system [mechanic]` — GDD for each MVP system; use prototype learnings
   in the Tuning Knobs and Formulas sections
6. `/aesir-review-all-gdds` — cross-system consistency check
...
the revised hypothesis for the next prototype. When `/aesir-prototype` is next run, check
...
- Run `/aesir-prototype [revised-concept]` to test the adjusted direction
- Or `/aesir-brainstorm [hint]` if the concept needs more fundamental rethinking
...
- Run `/aesir-brainstorm open` or `/aesir-brainstorm [new-hint]` to explore a different concept
...
2. **Choose path** — same clarify widget as Phase 3 (HTML / Engine / Paper).
