...
# Skill Test Spec: /aesir-prototype

## Skill Summary

`/aesir-prototype` manages a rapid prototyping workflow for validating a game mechanic
...
- [ ] Has a next-step handoff (e.g., `/aesir-design-system` to formalize, or archive)
...
**Input:** `/aesir-prototype grapple-hook`
...
**Input:** `/aesir-prototype grapple-hook` (prototype session complete)
...
2. Recommendation in findings.md: "Mechanic validated — recommend proceeding
   to `/aesir-design-system` for full specification"
3. Skill handoff message explicitly suggests `/aesir-design-system grapple-hook`
4. Verdict is PROTOTYPE COMPLETE

**Assertions:**
- [ ] `findings.md` contains an explicit recommendation
- [ ] Recommendation references `/aesir-design-system` when mechanic is validated
...
**Input:** `/aesir-prototype procedural-dialogue`
...
**Input:** `/aesir-prototype wall-jump`
