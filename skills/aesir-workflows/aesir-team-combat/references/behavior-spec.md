...
# Skill Test Spec: /aesir-team-combat
...
(parallel) → Integration → Validation → Sign-off. Uses `clarify` at each
phase transition. Delegates all file writes to sub-agents. Produces a summary report
with verdict COMPLETE / NEEDS WORK / BLOCKED and handoffs to `/aesir-code-review`,
`/aesir-balance-check`, and `/aesir-team-polish`.
...
- [ ] Has a next-step handoff at the end (references `/aesir-code-review`, `/aesir-balance-check`, `/aesir-team-polish`)
...
- [ ] Phase 2 includes spawning the primary engine specialist (read from `aesir-gameworks references/technical-preferences.md`)
...
- Engine is configured in `aesir-gameworks references/technical-preferences.md` (Engine Specialists section filled)
- No existing GDD for the requested combat feature

**Input:** `/aesir-team-combat parry and riposte system`
...
2. Phase 2 — gameplay-programmer + ai-programmer spawned; produce architecture sketch with class structure, interfaces, and file list; then primary engine specialist is spawned to validate idioms; engine specialist output incorporated; `clarify` presented with architecture options before Phase 3 begins
...
4. Phase 4 — integration wires together all Phase 3 outputs; tuning knobs verified as data-driven; `clarify` confirms integration before Phase 5
...
7. Next steps listed: `/aesir-code-review`, `/aesir-balance-check`, `/aesir-team-polish`
...
- [ ] Next steps include `/aesir-code-review`, `/aesir-balance-check`, `/aesir-team-polish`
...
4. `clarify` presented with options: (a) Skip ai-programmer and note the gap; (b) Retry with narrower scope; (c) Stop here and run `/aesir-architecture-decision` first
...
- [ ] `clarify` offers at minimum three options: skip / retry / stop
...
- [ ] Blocked reason references the ADR and suggests `/aesir-architecture-decision`
...
**Input:** `/aesir-team-combat parry and riposte system` (resuming from Phase 2 complete)
...
**Fixture:**
- `aesir-gameworks references/technical-preferences.md` has Engine Specialists section populated (e.g., Primary: godot-specialist)
...
2. Skill reads `aesir-gameworks references/technical-preferences.md` Engine Specialists section to identify the primary engine specialist agent type
...
**Assertions:**
- [ ] Engine specialist agent type is read from `aesir-gameworks references/technical-preferences.md` — not hardcoded
...
- [ ] Next steps present at end of output: `/aesir-code-review`, `/aesir-balance-check`, `/aesir-team-polish`
...
- "Retry with narrower scope" error recovery option is listed in assertions but its full
  recursive behavior (splitting via `/aesir-create-stories`) is covered by the `/aesir-create-stories` spec.
