...
> "Usage: `/aesir-team-combat [combat feature description]` — Provide a description of the combat feature to design and implement (e.g., `melee parry system`, `ranged weapon spread`)."
...
- **engine specialist** (primary) — Validate architecture and implementation patterns are idiomatic for the engine (read from `aesir-gameworks references/technical-preferences.md` Engine Specialists section)
...
- Run `/aesir-code-review` on the implemented combat code before closing stories.
- Run `/aesir-balance-check` to validate combat formulas and tuning values.
- Run `/aesir-team-polish` if VFX, audio, or performance polish is needed.
