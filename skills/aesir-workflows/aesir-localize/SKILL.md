...
### Issues
- [N] hardcoded strings found in source code (run /aesir-localize scan)
...
Read `aesir-gameworks references/technical-preferences.md` to determine the engine. Then check:
...
[ ] /aesir-localize scan shows zero hardcoded strings
[ ] /aesir-localize validate shows no placeholder mismatches in source (en)
...
## Post-Freeze Changes
[Any strings added or modified after freeze are listed here automatically by /aesir-localize extract]
```
...
- The list of all screens/flows in the game (from `design/gdd/` or `/aesir-content-audit` output)
- The current `/aesir-localize validate` report
...
```
/aesir-localize scan            → find hardcoded strings
/aesir-localize extract         → build string table
/aesir-localize freeze          → lock source before sending to translators
/aesir-localize brief           → generate translator briefing document
[Send to translators]
/aesir-localize validate        → check returned translations
/aesir-localize cultural-review → flag culturally sensitive content
/aesir-localize rtl-check       → if shipping Arabic / Hebrew / Persian
/aesir-localize vo-pipeline     → if shipping dubbed VO
/aesir-localize qa              → full localization QA pass
```

After `qa` returns PASS for all shipping locales, include the QA report path when running `/aesir-gate-check release`.
