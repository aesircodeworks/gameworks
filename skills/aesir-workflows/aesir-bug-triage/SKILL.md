...
- After `/aesir-team-qa` completes and new bugs have been filed
...
**Modes:**
- `/aesir-bug-triage sprint` — triage against the current sprint; assign fixable bugs
  to the sprint backlog; defer the rest
- `/aesir-bug-triage full` — full triage of all bugs regardless of sprint scope
- `/aesir-bug-triage trend` — trend analysis only (no assignment); read-only report
...
Read `aesir-gameworks references/coding-standards.md` for severity/priority definitions if they
...
- If any S1 bugs are unassigned: "S1 bugs must be assigned before the sprint
  can be considered healthy. Run `/aesir-sprint-status` to see current capacity."
- If regression bugs exist: "Regressions found — consider re-opening the
  affected stories in sprint tracking and running `/aesir-smoke-check` to re-gate."
