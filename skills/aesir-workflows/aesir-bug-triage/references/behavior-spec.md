...
# Skill Test Spec: /aesir-bug-triage

## Skill Summary

`/aesir-bug-triage` reads all open bug reports in `production/bugs/` and produces a
...
- [ ] Has a next-step handoff (e.g., `/aesir-bug-report` to create new reports, `/aesir-hotfix` for critical bugs)

---

## Director Gate Checks

None. `/aesir-bug-triage` is a read-only advisory skill. No director gates apply.
...
### Case 2: No Bug Reports Found — Guidance to run /aesir-bug-report
...
3. Skill suggests running `/aesir-bug-report` to create a bug report
4. No triage table is produced

**Assertions:**
- [ ] Output explicitly states no bugs were found
- [ ] `/aesir-bug-report` is suggested as the next step
