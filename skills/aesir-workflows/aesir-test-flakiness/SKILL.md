...
- After `/aesir-regression-suite` identifies quarantined tests that need diagnosis
...
**Modes:**
- `/aesir-test-flakiness [ci-log-path]` — analyse a specific CI run log file
- `/aesir-test-flakiness scan` — scan all available CI logs in `.github/` or
  standard log output directories
- `/aesir-test-flakiness registry` — read existing regression-suite.md quarantine
...
> 3. Run `/aesir-test-flakiness registry` to review tests already flagged as flaky
