...
Arguments can be combined: `/aesir-smoke-check sprint --platform console`
...
1. **Test framework check**: verify `tests/` directory exists.
   If it does not: "No test directory found at `tests/`. Run `/aesir-test-setup`
...
3. **Engine detection**: read `aesir-gameworks references/technical-preferences.md` and
...
5. **QA plan check**: glob `production/qa/qa-plan-*.md` and take the most
   ...
   `/aesir-qa-plan sprint` before smoke-checking for best results."
...
"Engine not configured in `aesir-gameworks references/technical-preferences.md`. Run
`/aesir-setup-engine` to specify the engine, then re-run `/aesir-smoke-check`."
...
3. If the `quick` argument was passed, skip this phase entirely and note:
   "Coverage scan skipped — run `/aesir-smoke-check sprint` for full coverage
   analysis."
...
appear prominently in the report and must be resolved before `/aesir-story-done` can
...
Use `clarify` to batch-verify. Keep to at most 3 calls.
...
**Engine**: [engine]
**QA Plan**: [path, or "Not found — run /aesir-qa-plan first"]
...
Fix the failures and run `/aesir-smoke-check` again to re-gate before QA hand-off."

**If verdict is PASS WITH WARNINGS:**

"Smoke check passed with warnings. The build is ready for manual QA.

Advisory items to resolve before running `/aesir-story-done` on affected stories:
