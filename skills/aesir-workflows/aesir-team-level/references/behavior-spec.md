...
# Skill Test Spec: /aesir-team-level
...
document saved to `design/levels/[level-name].md`. Uses `clarify` at each
step transition. Delegates all file writes to sub-agents. Produces a summary report
with verdict COMPLETE / BLOCKED and handoffs to `/aesir-design-review`, `/aesir-dev-story`,
`/aesir-qa-plan`.
...
- [ ] Has a next-step handoff at the end (references `/aesir-design-review`, `/aesir-dev-story`, `/aesir-qa-plan`)
...
**Input:** `/aesir-team-level forest dungeon`
...
2. Step 1 — narrative-director spawned: defines narrative purpose, key characters, dialogue triggers, emotional arc; world-builder spawned: provides lore context, environmental storytelling opportunities, world rules; `clarify` confirms Step 1 outputs before Step 2
3. Step 2 — level-designer spawned: designs spatial layout (critical path, optional paths, secrets), pacing curve, encounters, puzzles, entry/exit points and connections to adjacent areas; `clarify` confirms layout before Step 3
...
9. Next steps listed: `/aesir-design-review design/levels/forest-dungeon.md`, `/aesir-dev-story`, `/aesir-qa-plan`
...
- [ ] `clarify` called at each step gate (minimum: after Step 1, Step 2, Step 3, Step 4)
...
- [ ] Next steps include `/aesir-design-review`, `/aesir-dev-story`, `/aesir-qa-plan`
...
4. `clarify` presented with options:
...
**Input:** `/aesir-team-level` (no argument)
...
3. Provides example invocations: `/aesir-team-level tutorial`, `/aesir-team-level forest dungeon`, `/aesir-team-level final boss arena`
...
4. Skill presents both Step 4 results via `clarify` — BLOCKING concern highlighted prominently
5. `clarify` offers:
...
- [ ] `clarify` presents the specific concern text (not just "accessibility issue found")
...
4. `clarify` presented with options:
   ...
   - (b) Pause and run `/aesir-team-level crystal caves` first to establish that area
...
- [ ] `clarify` offers a "design crystal caves first" option referencing `/aesir-team-level`
...
- [ ] Next steps present at end: `/aesir-design-review`, `/aesir-dev-story`, `/aesir-qa-plan`
