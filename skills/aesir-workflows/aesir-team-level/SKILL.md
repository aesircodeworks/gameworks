...
**Gate**: Use `clarify` to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
- (b) Pause and run `/aesir-team-level [area-name]` first to establish that area

Do NOT invent content for the missing adjacent area.

**Gate**: Use `clarify` to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Use `clarify` to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Use `clarify` to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
...
- Run `/aesir-design-review design/levels/[level-name].md` to validate the completed level design doc.
- Run `/aesir-dev-story` to implement level content once the design is approved.
- Run `/aesir-qa-plan` to generate a QA test plan for this level.
