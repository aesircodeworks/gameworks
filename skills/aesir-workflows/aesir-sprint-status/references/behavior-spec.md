...
# Skill Test Spec: /aesir-sprint-status

## Skill Summary

`/aesir-sprint-status` is a Haiku-tier read-only skill that reads the current active
...
None. `/aesir-sprint-status` is a read-only reporting skill; no gates are invoked.
...
3. Skill suggests running `/aesir-milestone-review` or `/aesir-sprint-plan` as next steps

**Assertions:**
- [ ] Verdict is ON TRACK or SPRINT COMPLETE when all stories are Complete
- [ ] Output notes that the sprint is fully done
- [ ] Next-step suggestion references `/aesir-milestone-review` or `/aesir-sprint-plan`
- [ ] No files are written

---

### Case 3: No Active Sprint File — Guidance to run /aesir-sprint-plan
...
4. Skill suggests running `/aesir-sprint-plan` to create one

**Assertions:**
- [ ] Skill does not error or crash when no sprint file exists
- [ ] Output clearly states no active sprint was found
- [ ] Output recommends `/aesir-sprint-plan` as the next action
