...
`/aesir-sprint-plan update` or `/aesir-milestone-review`.
...
- If an argument is given (e.g., `/aesir-sprint-status 3`), search
...
- If `production/sprints/` does not exist or is empty, report: "No sprint
  files found. Start a sprint with `/aesir-sprint-plan new`." Then stop.
...
"⚠ No `sprint-status.yaml` found — status inferred from markdown. Run `/aesir-sprint-plan update` to generate one."
...
time remaining. Recommend replanning with `/aesir-sprint-plan update`.
```
...
Run `/aesir-story-readiness sprint` to validate story file coverage.
```
...
- It does not propose scope cuts (that is `/aesir-sprint-plan update`)
- It makes at most one recommendation per run

For more detail on a specific story, the user can read the story file directly
or run `/aesir-story-readiness [path]`.

For sprint replanning, use `/aesir-sprint-plan update`.
For end-of-sprint retrospective, use `/aesir-retrospective`.
