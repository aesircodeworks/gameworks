...
- If confidence is Low: recommend a time-boxed spike (`/aesir-prototype`) before committing.
- If the task is > 10 days: recommend breaking it into smaller stories via `/aesir-create-stories`.
- To schedule the task: run `/aesir-sprint-plan update` to add it to the next sprint.
