...
# Skill Test Spec: /aesir-day-one-patch

## Skill Summary

`/aesir-day-one-patch` prepares a day-one patch plan for issues that are known at
...
triggers guidance to run `/aesir-hotfix` before the patch. No director gates apply.
...
- [ ] Has a next-step handoff (e.g., `/aesir-hotfix` for P0 issues, `/aesir-release-checklist` for follow-up)

---

## Director Gate Checks

None. `/aesir-day-one-patch` is a release planning utility. No director gates apply.
...
### Case 2: Critical Issue Discovered Post-Ship — P0, Triggers /aesir-hotfix Guidance
...
4. Skill explicitly directs: "Run `/aesir-hotfix` to resolve this issue first"
...
- [ ] `/aesir-hotfix` is explicitly directed for the P0 issue
...
- [ ] Escalates CRITICAL (P0) bugs with explicit `/aesir-hotfix` guidance
...
- The patch notes player communication document (`/aesir-patch-notes`) is a separate
