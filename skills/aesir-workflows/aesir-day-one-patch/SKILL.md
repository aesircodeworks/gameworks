...
If [C]: output "No day-one patch required. Proceed to `/aesir-launch-checklist`." Stop.
...
Use `clarify` to confirm proceeding or reduce scope.
...
This is a lightweight QA pass — not a full `/aesir-team-qa`. The patch is already QA-approved from the release gate; we are only re-verifying the changed areas.
...
- **Targeted smoke check** — run `/aesir-smoke-check [affected-systems]`
...
1. Run `/aesir-patch-notes` to generate the player-facing version of the patch notes
2. Run `/aesir-bug-report verify [BUG-ID]` for each fixed bug after the patch is live
3. Run `/aesir-bug-report close [BUG-ID]` for each verified fix
4. Schedule a post-launch review 48–72 hours after launch using `/aesir-retrospective launch`
...
- Options:
  - `[A] Run /aesir-patch-notes — generate player-facing patch notes`
  - `[B] Run /aesir-bug-report to log any issues found post-deploy`
  - `[C] Stop here`
...
- **Player communication is part of the patch** — `/aesir-patch-notes` is a required output, not optional
