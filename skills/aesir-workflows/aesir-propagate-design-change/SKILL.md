---
name: aesir-propagate-design-change
...
**Usage:** `/aesir-propagate-design-change design/gdd/combat-system.md`
...
> "Usage: `/aesir-propagate-design-change design/gdd/[system].md`
...
Spawn `technical-director` via Task using gate **TD-CHANGE-IMPACT** (`aesir-gameworks references/director-gates.md`).
...
- **CONCERNS** → surface the specific ADRs or recommendations flagged; use `clarify` with options: `Revise the impact assessment` / `Accept with noted concerns` / `Discuss further`
...
- **ADRs marked Superseded**: "Run `/aesir-architecture-decision [title]` to write the
  replacement ADR. Then re-run `/aesir-propagate-design-change` to verify coverage."
- **ADRs to update in place**: List the specific fields to update in each ADR
- **If many ADRs affected**: "Run `/aesir-architecture-review` after all ADRs are updated
