...
the `/aesir-team-ui` pipeline.

**Run this skill:**
- After completing a UX spec with `/aesir-ux-design`
...
- **Specific file path** (e.g., `/aesir-ux-review design/ux/inventory.md`): validate
  that one document
...
1. **Input & Platform config**: Read `aesir-gameworks references/technical-preferences.md` and
...
[For APPROVED]: This spec is ready for handoff to `/aesir-team-ui` Phase 2
(Visual Design).
...
Recommend returning to `/aesir-ux-design` to rework [sections].
```
...
- For **APPROVED**: suggest running `/aesir-team-ui` to begin implementation coordination
- For **NEEDS REVISION**: offer to help fix specific gaps ("Would you like me to
  ...
  instruction
- For **MAJOR REVISION NEEDED**: suggest returning to `/aesir-ux-design` with the
