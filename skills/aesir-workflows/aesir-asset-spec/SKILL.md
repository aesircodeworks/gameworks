...
Present the full proposed inventory to the user in conversation. Then use `clarify`:
...
> - Run `/aesir-ux-design [screen name]` for each UI screen in the inventory
> - Run `/aesir-asset-spec entity:[name]` to spec each visual entity
> - Or run `/aesir-asset-spec` again to work through the inventory one item at a time"
...
- **Art bible**: Read `design/art/art-bible.md` — fail if missing:
  > "No art bible found. Run `/aesir-art-bible` first — asset specs are anchored to the art bible's visual rules and asset standards."
...
- **Technical preferences**: Read `aesir-gameworks references/technical-preferences.md` — extract performance budgets and naming conventions.
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  > "The Visual/Audio section of `design/gdd/[target-name].md` is empty. Either run `/aesir-design-system [target-name]` to complete the GDD, or describe the visual needs manually."
  Use `clarify`: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, use `clarify`:
...
Present the full identified list to the user. Use `clarify`:
...
- Options:
  - `[A] Spec another system — /aesir-asset-spec system:[next-system]`
  - `[B] Spec a level — /aesir-asset-spec level:[level-name]`
  - `[C] Spec a character — /aesir-asset-spec character:[character-name]`
  - `[D] Run /aesir-asset-audit — validate delivered assets against specs`
  - `[E] Stop here`
...
- Run `/aesir-asset-spec [next-context]` to continue speccing remaining systems, levels, or characters
- Run `/aesir-asset-audit` to validate delivered assets against the written specs and identify gaps or mismatches
