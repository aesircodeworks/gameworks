...
# Skill Test Spec: /aesir-quick-design

## Skill Summary

`/aesir-quick-design` produces a lightweight design spec for features too small to
...
for a quick-design, the skill redirects to `/aesir-design-system` instead.
...
- [ ] Mentions scope check: redirects to `/aesir-design-system` if scope exceeds sub-4h threshold
...
**Input:** `/aesir-quick-design [feature-name]`
...
### Case 2: Failure Path — Scope check fails; redirected to /aesir-design-system
...
**Input:** `/aesir-quick-design` (no argument)

**Expected behavior:**
1. Skill detects no argument is provided
2. Skill outputs a usage error: "No feature name specified. Usage: /aesir-quick-design [feature-name]"
3. Skill provides an example: `/aesir-quick-design pause-menu-settings`
...
- [ ] Full GDD review path (`/aesir-design-system`) is mentioned as the alternative for larger features

---

## Protocol Compliance

- [ ] Scope check runs before drafting (redirects to `/aesir-design-system` if scope too large)
...
- [ ] Ends with next-step handoff (e.g., proceed to implementation or `/aesir-dev-story`)
