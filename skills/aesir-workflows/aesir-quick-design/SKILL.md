...
Full GDD authoring via `/aesir-design-system` is the heavyweight path. Use this skill
...
**When to run:** Anytime a change is too small for `/aesir-design-system` but too
...
- Options:
  ...
  - `[F] This is too large — redirect me to /aesir-design-system`

If [F]: stop. Verdict: **REDIRECTED** — use `/aesir-design-system` for this change.
...
- Options:
  ...
  - `[C] This grew too large — redirect to /aesir-design-system instead`

If [B]: collect the requested changes, revise the draft, and re-present this widget.
If [C]: stop. Verdict: **REDIRECTED** — use `/aesir-design-system` for this change.
...
Next step: This spec is ready for `/aesir-story-readiness` validation before
...
Quick Design Specs **bypass** `/aesir-design-review` and `/aesir-review-all-gdds` by
...
using `/aesir-design-system` to author a full GDD for this."

---

## Recommended Next Steps

- Run `/aesir-story-readiness [story-path]` to validate the story before implementation begins — reference this spec in the story's GDD Reference field
- Run `/aesir-dev-story [story-path]` to implement once the story passes readiness checks
- If the change is larger than expected, run `/aesir-design-system [system-name]` to author a full GDD instead
