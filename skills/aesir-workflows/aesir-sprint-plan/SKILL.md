...
- If the file doesn't exist and this is a `new` sprint: use `clarify`:
...
- [ ] Smoke check passed (`/aesir-smoke-check sprint`)
- [ ] QA sign-off report: APPROVED or APPROVED WITH CONDITIONS (`/aesir-team-qa sprint`)
...
3. Ask the user what to change: stories to add, remove, reprioritize, or re-estimate. Use `clarify` to gather changes.
...
`/aesir-sprint-status`, `/aesir-story-done`, and `/aesir-help` without markdown parsing.
...
```yaml
# Auto-generated by /aesir-sprint-plan. Updated by /aesir-story-done and /aesir-dev-story.
# DO NOT edit manually — use /aesir-story-done to update story status.
#
...
Before finalising the sprint plan, spawn `producer` via Task using gate **PR-SPRINT** (`aesir-gameworks references/director-gates.md`).
...
> **Scope check:** If this sprint includes stories added beyond the original epic scope, run `/aesir-scope-check [epic]` to detect scope creep before implementation begins.
...
Use `search_files` to look for `production/qa/qa-plan-sprint-[N].md` or any file in `production/qa/` referencing this sprint number.
...
>
> Run `/aesir-qa-plan sprint` now, before starting any implementation. It takes one session and produces the test case requirements each story needs."

Use `clarify`:
- Prompt: "No QA plan found for this sprint. How do you want to proceed?"
- Options:
  - `[A] Run /aesir-qa-plan sprint now — I'll do that before starting implementation (Recommended)`
  - `[B] Skip for now — I understand QA sign-off will be blocked at the Production → Polish gate`

If [A]: close with "Sprint plan written. Run `/aesir-qa-plan sprint` next — then begin implementation."
If [B]: add a warning block to the sprint plan document:

```markdown
> ⚠️ **No QA Plan**: This sprint was started without a QA plan. Run `/aesir-qa-plan sprint`
...
- `/aesir-qa-plan sprint` — **required before implementation begins** — defines test cases per story so developers implement against QA specs, not a blank slate
- `/aesir-story-readiness [story-file]` — validate a story is ready before starting it
- `/aesir-dev-story [story-file]` — begin implementing the first story
- `/aesir-sprint-status` — check progress mid-sprint
- `/aesir-scope-check [epic]` — verify no scope creep before implementation begins
...
This file is read by `/aesir-sprint-plan`, `/aesir-story-readiness`, `/aesir-story-done`, and other skills at startup.
