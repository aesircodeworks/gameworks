...
Before generating the Go/No-Go recommendation, spawn `producer` via Task using gate **PR-MILESTONE** (`aesir-gameworks references/director-gates.md`).
...
If OFF TRACK, use `clarify` before generating the recommendation:
...
- Run `/aesir-gate-check` for a formal phase gate verdict if this milestone marks a development phase boundary.
- Run `/aesir-sprint-plan` to adjust the next sprint based on the scope recommendations above.
