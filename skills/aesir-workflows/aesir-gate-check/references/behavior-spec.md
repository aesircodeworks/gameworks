...
# Skill Test Spec: /aesir-gate-check

## Skill Summary

`/aesir-gate-check` validates whether the project is ready to advance to the next
...
**Input:** `/aesir-gate-check systems-design`
...
5. Skill suggests remediation: run `/aesir-brainstorm` to create one
...
- [ ] Output recommends `/aesir-brainstorm` as the remediation action
...
**Input:** `/aesir-gate-check` (no argument)
...
**Input:** `/aesir-gate-check systems-design` (with full mode active)
...
**Input:** `/aesir-gate-check systems-design` (with solo mode active)
...
clarify to confirm the auto-detected transition before running full checks —
