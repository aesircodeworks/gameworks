...
**Distinct from `/aesir-project-stage-detect`**: That skill is diagnostic ("where are we?").
...
- **With argument**: `/aesir-gate-check production` — validate readiness for that specific phase
- **No argument**: Auto-detect current stage using the same heuristics as
  `/aesir-project-stage-detect`, then **confirm with the user before running**:
...
- [ ] Concept prototype exists in `prototypes/` with a REPORT.md showing PROCEED verdict
      (`/aesir-prototype [core-mechanic]`) — skipping this means GDDs may be written for an
      idea that hasn't been played. Acceptable if the concept is proven by other means.

**Quality Checks:**
- [ ] Game concept has been reviewed (`/aesir-design-review` verdict not MAJOR REVISION NEEDED)
...
- [ ] All MVP-tier GDDs exist in `design/gdd/` and individually pass `/aesir-design-review`
- [ ] A cross-GDD review report exists in `design/gdd/` (from `/aesir-review-all-gdds`)

**Quality Checks:**
- [ ] All MVP GDDs pass individual design review (8 required sections, no MAJOR REVISION NEEDED verdict)
- [ ] `/aesir-review-all-gdds` verdict is not FAIL (cross-GDD consistency and design theory checks pass)
- [ ] All cross-GDD consistency issues flagged by `/aesir-review-all-gdds` are resolved or explicitly accepted
...
- [ ] Technical preferences configured (`aesir-gameworks references/technical-preferences.md` populated)
...
- [ ] `/aesir-architecture-review` has been run (a review report file exists in `docs/architecture/`)
...
- [ ] `/aesir-architecture-review` engine audit shows no deprecated API usage
...
**Required Artifacts:**
- [ ] Vertical slice exists in `prototypes/` with a REPORT.md (run `/aesir-vertical-slice`) — **recommended, not blocking**; if absent, surface as CONCERNS
...
- [ ] Entity inventory exists at `design/assets/entity-inventory.md` (recommended — run `/aesir-asset-spec` with no arguments to generate collaboratively from GDDs + art bible)
...
- [ ] Control manifest exists at `docs/architecture/control-manifest.md`
      (generated by `/aesir-create-control-manifest` from Accepted ADRs)
- [ ] Epics defined in `production/epics/` with at least Foundation and Core
      layer epics present (use `/aesir-create-epics layer: foundation` and
      `/aesir-create-epics layer: core` to create them, then `/aesir-create-stories [epic-slug]`
      for each epic)
...
- [ ] All key screen UX specs have passed `/aesir-ux-review` (verdict APPROVED or NEEDS REVISION accepted)
...
- [ ] Manual validation confirms GDDs + architecture + epics are coherent
      (run `/aesir-review-all-gdds` and `/aesir-architecture-review` if not done recently)
...
- [ ] QA plan exists in `production/qa/` (generated by `/aesir-qa-plan`) covering this sprint or final production sprint
- [ ] At least one QA plan exists in `production/qa/` covering this production phase — run `/aesir-qa-plan` if missing (CONCERNS — advisory, not blocking)
- [ ] QA sign-off report exists in `production/qa/` (generated by `/aesir-team-qa`) with verdict APPROVED or APPROVED WITH CONDITIONS
...
- [ ] QA test plan exists (`/aesir-qa-plan` output in `production/qa/`)
- [ ] QA sign-off report exists (`/aesir-team-qa` output — APPROVED or APPROVED WITH CONDITIONS)
...
- [ ] Balance data has been reviewed (`/aesir-balance-check` run)
- [ ] Release checklist completed (`/aesir-release-checklist` or `/aesir-launch-checklist` run)
...
### Artifact Checks
- Use `search_files` and `read_file` to verify files exist and have meaningful content
...
Use `Glob('design/gdd/gdd-cross-review-*.md')` to find the `/aesir-review-all-gdds` report.
If no file matches, mark the "cross-GDD review report exists" artifact as **FAIL** and
surface it prominently: "No `/aesir-review-all-gdds` report found in `design/gdd/`. Run
`/aesir-review-all-gdds` before advancing to Technical Setup."
...
### Quality Checks
- For test checks: Run the test suite via `terminal` if a test runner is configured
- For design review checks: `read_file` the GDD and check for the 8 required sections
- For performance checks: `read_file` technical-preferences.md and compare against any
  profiling data in `tests/performance/` or recent `/aesir-perf-profile` output
- For localization checks: `search_files` for hardcoded strings in `src/`
...
- "Performance profiling data isn't available. Would you like to run `/aesir-perf-profile`?"
...
Before generating the final verdict, spawn all four directors as **parallel subagents** via Task using the parallel gate protocol from `aesir-gameworks references/director-gates.md`. Issue all four Task calls simultaneously — do not wait for one before starting the next.

**Spawn in parallel:**

1. **`creative-director`** — gate **CD-PHASE-GATE** (`aesir-gameworks references/director-gates.md`)
2. **`technical-director`** — gate **TD-PHASE-GATE** (`aesir-gameworks references/director-gates.md`)
3. **`producer`** — gate **PR-PHASE-GATE** (`aesir-gameworks references/director-gates.md`)
4. **`art-director`** — gate **AD-PHASE-GATE** (`aesir-gameworks references/director-gates.md`)
...
### Blockers
1. **No Architecture Decision Records** — Run `/aesir-architecture-decision` to create one
...
[A] Run /aesir-create-architecture — produce your master architecture blueprint and ADR work plan (recommended next step)
[B] Design more GDDs first — return here when all MVP systems are complete
[C] Stop here for this session
```

> **Note for systems-design PASS**: `/aesir-create-architecture` is the required next step before writing any ADRs. It produces the master architecture document and a prioritized list of ADRs to write. Running `/aesir-architecture-decision` without this step means writing ADRs without a blueprint — skip it at your own risk.

For **technical-setup PASS**:
```
Gate passed. What would you like to do next?
[A] Run /aesir-create-control-manifest — generate the layer rules manifest from your Accepted ADRs (do this first)
[B] Run /aesir-vertical-slice — build the Vertical Slice (do this before writing epics — validate fun first)
[C] Write more ADRs first — run /aesir-architecture-decision [next-system]
...
>
> 1. `/aesir-create-control-manifest` — extract technical rules from Accepted ADRs (required before epics)
> 2. `/aesir-vertical-slice` — build the Vertical Slice **FIRST**, before writing epics or stories
> 3. Playtest → `/aesir-playtest-report` — at least 1 session required to pass the Pre-Production gate; 3+ recommended before committing the full team
> 4. `/aesir-ux-design [screen]` — UX specs for main menu, core HUD, pause menu (if not done)
> 5. `/aesir-create-epics layer:foundation` then `/aesir-create-epics layer:core` — plan after fun is validated
> 6. `/aesir-create-stories [epic-slug]` for each epic
> 7. `/aesir-sprint-plan new`
>
...
- **No art bible?** → `/aesir-art-bible` to create the visual identity specification
- **Art bible exists but no asset specs?** → `/aesir-asset-spec system:[name]` to generate per-asset visual specs and generation prompts from approved GDDs
- **No game concept?** → `/aesir-brainstorm` to create one
- **No systems index?** → `/aesir-map-systems` to decompose the concept into systems
- **Missing design docs?** → `/aesir-reverse-document` or delegate to `game-designer`
- **Small design change needed?** → `/aesir-quick-design` for changes under ~4 hours (bypasses full GDD pipeline)
- **No UX specs?** → `/aesir-ux-design [screen name]` to author specs, or `/aesir-team-ui [feature]` for full pipeline
- **UX specs not reviewed?** → `/aesir-ux-review [file]` or `/aesir-ux-review all` to validate
- **No accessibility requirements doc?** → run `/aesir-ux-design` which creates both `design/accessibility-requirements.md` and `design/ux/interaction-patterns.md` in one step
- **No interaction pattern library?** → `/aesir-ux-design patterns` to initialize it
- **GDDs not cross-reviewed?** → `/aesir-review-all-gdds` (run after all MVP GDDs are individually approved)
- **Cross-GDD consistency issues?** → fix flagged GDDs, then re-run `/aesir-review-all-gdds`
- **No test framework?** → `/aesir-test-setup` to scaffold the framework for your engine
- **No QA plan for current sprint?** → `/aesir-qa-plan sprint` to generate one before implementation begins
- **Missing ADRs?** → `/aesir-architecture-decision` for individual decisions
- **No master architecture doc?** → `/aesir-create-architecture` for the full blueprint
- **ADRs missing engine compatibility sections?** → Re-run `/aesir-architecture-decision`
  or manually add Engine Compatibility sections to existing ADRs
- **Missing control manifest?** → `/aesir-create-control-manifest` (requires Accepted ADRs)
- **Missing epics?** → `/aesir-create-epics layer: foundation` then `/aesir-create-epics layer: core` (requires control manifest)
- **Missing stories for an epic?** → `/aesir-create-stories [epic-slug]` (run after each epic is created)
- **Stories not implementation-ready?** → `/aesir-story-readiness` to validate stories before developers pick them up
- **Tests failing?** → delegate to `lead-programmer` or `qa-tester`
- **No playtest data?** → `/aesir-playtest-report`
- **No playtest sessions beyond the minimum?** → Additional sessions give more reliable signal. 3+ total is recommended before committing the full team. Use `/aesir-playtest-report` to structure findings.
- **No Difficulty Curve doc?** → Create `design/difficulty-curve.md` from the template at `aesir-project-templates templates/difficulty-curve.md` — or use `/aesir-quick-design "difficulty curve"` for a guided session.
- **No player journey map?** → Create `design/player-journey.md` from the template at `aesir-project-templates templates/player-journey.md` — or author it collaboratively using `/aesir-ux-design` Phase 2b.
- **Need a quick sprint check?** → `/aesir-sprint-status` for current sprint progress snapshot
- **Performance unknown?** → `/aesir-perf-profile`
- **Not localized?** → `/aesir-localize`
- **Ready for release?** → `/aesir-launch-checklist`
...
6. **Never auto-fix**: If required artifacts are missing, report the FAIL verdict and
   name the skill to run (e.g. "run `/aesir-test-setup`"). Do NOT create missing files or
