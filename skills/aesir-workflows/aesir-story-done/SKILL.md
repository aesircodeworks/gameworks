...
**If a file path is provided** (e.g., `/aesir-story-done production/epics/core/story-damage-calculator.md`):
...
3. If multiple in-progress stories are found, use `clarify`:
...
- **File existence check**: `search_files` for files the story said would be created.
- **Test pass check**: if a test file path is mentioned, run it via `terminal`.
- **No hardcoded values check**: `search_files` for numeric literals in gameplay code
  paths that should be in config files.
- **No hardcoded strings check**: `search_files` for player-facing strings in `src/`
  that should be in localization files.
- **Dependency check**: if a criterion says "depends on X", check that X exists.

### Manual verification with confirmation (use `clarify`)
...
Batch up to 4 manual verification questions into a single `clarify` call:
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **Unit test**: check `tests/unit/` for a test file or function name that
     matches the criterion's subject (use `search_files` and `search_files`)
   - **Integration test**: check `tests/integration/` similarly
   - **Manual confirmation**: if the criterion was verified via `clarify`
...
exact required file path. Use `search_files` to check that exact path. If the exact path is not
...
required path. Use `search_files` to check that exact path first, then search
...
If none: flag as **ADVISORY** — "No smoke check report found. Run `/aesir-smoke-check`."
...
1. **GDD rules check**: Using the current requirement text from `tr-registry.yaml`
   ...
   `search_files` the implemented files for key function names, data structures, or class
...
2. **Manifest version staleness check**: Compare the `Manifest Version:` date
   ...
   - If the story's version is older → flag as ADVISORY:
     ...
     is v[current-date]. New rules may apply. Run /aesir-story-readiness to check.`
...
3. **ADR constraints check**: Read the referenced ADR's Decision section. Check
   ...
   exists). `search_files` for patterns explicitly forbidden in the ADR.

4. **Hardcoded values check**: `search_files` the implemented files for numeric literals
...
After completing the deviation checks in Phase 4, spawn `qa-lead` via Task using gate **QL-TEST-COVERAGE** (`aesir-gameworks references/director-gates.md`).
...
- `lean` → use `clarify` before proceeding:
  - Prompt: "Code review is skipped in lean mode. Did you run `/aesir-code-review` on the implemented files?"
  - Options:
    - `Yes — /aesir-code-review passed or was approved with suggestions`
    - `No — skipping code review for this story`
    - `No — I'll run /aesir-code-review before the sprint close-out`
  - Record the answer in the completion notes (Phase 7). All three options proceed to Phase 6.
- `full` → spawn as normal.

Spawn `lead-programmer` via Task using gate **LP-CODE-REVIEW** (`aesir-gameworks references/director-gates.md`).

Pass: implementation file paths, story file path, relevant GDD section, governing ADR.

Present the verdict to the user. If CONCERNS, surface them via `clarify`:
...
Use `clarify` before writing anything:
...
`production/session-state/active.md`:
    ...
    ## Session Extract — /aesir-story-done [date]
    - Verdict: [COMPLETE / COMPLETE WITH NOTES / BLOCKED]
...
Run `/aesir-story-readiness [path]` to confirm a story is implementation-ready
before starting.
```
...
1. `/aesir-smoke-check sprint` — verify the critical path still works end-to-end
2. `/aesir-team-qa sprint` — full QA cycle: test case execution, bug triage, sign-off report
3. `/aesir-retrospective` — capture what went well, what didn't, and action items for the next sprint
4. `/aesir-gate-check` — advance to the next phase once QA approves (only if advancing a phase)
5. `/aesir-sprint-plan new` — plan the next sprint, incorporating velocity data and retrospective action items

Do not run `/aesir-gate-check` until `/aesir-team-qa` returns APPROVED or APPROVED WITH CONDITIONS.
```
...
- Use `clarify` for the code review prompt and for batching manual
...
- Run `/aesir-story-readiness [next-story-path]` to validate the next story before starting implementation
- If all Must Have stories are complete: run `/aesir-smoke-check sprint` → `/aesir-team-qa sprint` → `/aesir-gate-check`
- If tech debt was logged: track it via `/aesir-tech-debt` to keep the register current
