...
**If no argument is provided**, do not fail — ask instead. Use `clarify`:
...
- **Game concept**: Read `design/gdd/game-concept.md` — if missing, warn:
  > "No game concept found. Run `/aesir-brainstorm` first to establish the game's
...
> "Player journey map not yet created. Template available at `aesir-project-templates templates/player-journey.md`. Run `/aesir-ux-design` Phase 2b or create it manually to establish player context for this screen."
...
Read `aesir-gameworks references/technical-preferences.md` and extract the `## Input & Platform`
...
>
> (Run `/aesir-setup-engine` to save this permanently so you won't be asked again.)
...
- Use `patch` to fill placeholders in-place rather than creating a new skeleton
...
2. **Questions**: Ask what is needed to draft this section. Use `clarify`
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use `clarify` to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use `clarify`:
...
7. **Write**: Use `clarify`: "May I write the [section name] section to `[filepath]`?"
   - Options: "Yes, write it", "Wait — one more change"
   Once confirmed, use `patch` to replace the `[To be designed]` placeholder with approved content.
...
- Use `clarify` to capture the choice:
...
- Use `clarify`: "Want an ASCII wireframe as part of this spec?"
...
> "Accessibility tier not yet defined — consider WCAG-AA as a baseline. Run `/aesir-gate-check` to see whether this blocks any phase gates."
...
Write at least 5 specific, testable criteria that a QA tester can verify without reading any other design document. These become the pass/fail conditions for `/aesir-story-done`.
...
Use `clarify` to step through items in groups of 3-4, not all at once.
...
Work through patterns in groups. Use `clarify`:
...
> "This spec should be validated with `/aesir-ux-review` before it enters the
> implementation pipeline. The Pre-Production gate requires all key screen specs
> to have a review verdict."

Then use `clarify`:
- "Run `/aesir-ux-review [filename]` now, or do something else first?"
  - Options:
    - "Run `/aesir-ux-review` now — validate this spec"
...
`/aesir-ux-review` on all completed specs before running `/aesir-gate-check pre-production`."
...
When delegating to another agent via the delegate_task tool:
...
- Run `/aesir-ux-review [filename]` to validate this spec before it enters the implementation pipeline
- Run `/aesir-ux-design [next-screen]` to continue designing remaining screens or flows
- Run `/aesir-gate-check pre-production` once all key screens have approved UX specs
