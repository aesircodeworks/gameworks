---
name: aesir-test-evidence-review
...
`/aesir-smoke-check` verifies that test files **exist** and **pass**. This skill
...
**When to run:**
- Before QA hand-off sign-off (`/aesir-team-qa` Phase 5)
...
**Modes:**
- `/aesir-test-evidence-review [story-path]` — review a single story's evidence
- `/aesir-test-evidence-review sprint` — review all stories in the current sprint
- `/aesir-test-evidence-review [system-name]` — review all stories in an epic/system
...
- For BLOCKING items: "These must be resolved before `/aesir-story-done` can mark the
  story Complete. Would you like to address any of them now?"
- For thin assertions: "Consider running `/aesir-test-helpers [system]` to see
