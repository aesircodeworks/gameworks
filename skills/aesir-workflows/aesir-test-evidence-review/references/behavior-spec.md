...
# Skill Test Spec: /aesir-test-evidence-review

## Skill Summary

`/aesir-test-evidence-review` performs a quality review of test files in `tests/`,
...
**Input:** `/aesir-test-evidence-review tests/unit/combat/`
...
**Input:** `/aesir-test-evidence-review tests/unit/ui/hud_update_test.gd`
...
**Input:** `/aesir-test-evidence-review tests/unit/networking/auth_test.gd`
...
**Fixture:**
- User calls `/aesir-test-evidence-review tests/unit/audio/`
- `tests/unit/audio/` directory does not exist

**Input:** `/aesir-test-evidence-review tests/unit/audio/`

**Expected behavior:**
1. Skill attempts to read files in `tests/unit/audio/` — not found
2. Skill outputs: "No test files found at `tests/unit/audio/` — run `/aesir-test-setup` to scaffold test directories"
3. No verdict is emitted

**Assertions:**
- [ ] Skill does not crash when path does not exist
- [ ] Output names the attempted path in the message
- [ ] Output recommends `/aesir-test-setup` for scaffolding
...
4. Output notes: "For full test coverage gate, run `/aesir-gate-check` which invokes QL-TEST-COVERAGE"
