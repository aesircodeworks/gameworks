...
|------|---------|---------|------------|
| `static` | `/aesir-skill-test static [name\|all]` | Structural linter — 7 compliance checks per skill | Low (~1k/skill) |
| `spec` | `/aesir-skill-test spec [name]` | Behavioral verifier — evaluates assertions in test spec | Medium (~5k/skill) |
| `category` | `/aesir-skill-test category [name\|all]` | Category rubric — checks skill against its category-specific metrics | Low (~2k/skill) |
| `audit` | `/aesir-skill-test audit` | Coverage report — skills, agent specs, last test dates | Low (~3k total) |
...
- `category [name]` → run category-specific rubric from `aesir-framework-qa references/quality-rubric.md`
...
- A final section mentioning another skill (e.g., `/aesir-story-done`, `/aesir-gate-check`)
...
Look up the spec path from `aesir-framework-qa references/catalog.yaml` — use the
...
- Spec file not found at path: "Spec file missing at [path]. Run `/aesir-skill-test audit`
...
"May I write these results to `aesir-framework-qa references/results/skill-test-spec-[name]-[date].md`
and update `aesir-framework-qa references/catalog.yaml`?"

If yes:
- Write results file to `aesir-framework-qa references/results/`
- Update the skill's entry in `aesir-framework-qa references/catalog.yaml`:
  - `last_spec: [date]`
...
Find skill at `(game-workspace)/skills/[name]/SKILL.md`.
Look up `category:` field in `aesir-framework-qa references/catalog.yaml`.
...
Read `aesir-framework-qa references/quality-rubric.md`.
...
"May I update `aesir-framework-qa references/catalog.yaml` to record this category check
...
Read `aesir-framework-qa references/catalog.yaml`. If missing, note that catalog doesn't exist
yet (first-run state).
...
Also read the `agents:` section from `aesir-framework-qa references/catalog.yaml` to get the
...
- Check if a spec file exists (use the `spec:` path from catalog, or glob `aesir-framework-qa references/agents/*/[name].md`)
...
Offer: "Would you like to run `/aesir-skill-test static all` to check structural
compliance across all skills? `/aesir-skill-test category all` to run category rubric
checks? Or `/aesir-skill-test spec [name]` to run a specific behavioral test?"
...
- After `static [name]`: "Run `/aesir-skill-test spec [name]` to validate behavioral
  correctness if a test spec exists."
- After `static all` with failures: "Address NON-COMPLIANT skills first. Run
  `/aesir-skill-test static [name]` individually for detailed remediation guidance."
- After `spec [name]` PASS: "Update `aesir-framework-qa references/catalog.yaml` to record this
  pass date. Consider running `/aesir-skill-test audit` to find the next spec gap."
...
- After `audit`: "Start with the critical-priority gaps. Use the spec template
  at `aesir-framework-qa references/templates/skill-test-spec.md` to create new specs."
