...
# Skill Test Spec: /aesir-skill-test

## Skill Summary

`/aesir-skill-test` validates skill files for structural correctness, behavioral
...
- **audit**: Produces a coverage table of all skills in `(game-workspace)/skills/` and
  all agents in `(game-workspace)/agents/`, showing which have spec files and which do not.
...
- [ ] Has a next-step handoff (e.g., `/aesir-skill-improve` to fix issues found)

---

## Director Gate Checks

None. `/aesir-skill-test` is a meta-utility skill. No director gates apply.
...
**Fixture:**
- `skill_view('aesir-brainstorm')/SKILL.md` exists and is well-formed:
...
**Input:** `/aesir-skill-test static brainstorm`

**Expected behavior:**
1. Skill reads `skill_view('aesir-brainstorm')/SKILL.md`
...
**Input:** `/aesir-skill-test static some-skill`
...
- `skill_view('aesir-gate-check')/SKILL.md` exists

**Input:** `/aesir-skill-test spec gate-check`
...
**Fixture:**
- `(game-workspace)/skills/` contains 72+ skill directories
- `(game-workspace)/agents/` contains 49+ agent files
- `tests/skills/` contains spec files for a subset of skills

**Input:** `/aesir-skill-test audit`

**Expected behavior:**
1. Skill enumerates all skills in `(game-workspace)/skills/` and all agents in `(game-workspace)/agents/`
...
- `skill_view('aesir-gate-check')/SKILL.md` is a gate skill

**Input:** `/aesir-skill-test category gate-check`
...
- [ ] Suggests `/aesir-skill-improve` as the next step when issues are found
