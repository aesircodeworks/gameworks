...
- **Design changes:** "Run `/aesir-propagate-design-change [path]` on the affected design document to find downstream impacts before making changes."
- **Balance adjustments:** "Run `/aesir-balance-check [system]` to verify the full balance picture before tuning values."
- **Bugs:** "Use `/aesir-bug-report` to formally track these."
...
After categorising findings, spawn `creative-director` via Task using gate **CD-PLAYTEST** (`aesir-gameworks references/director-gates.md`).
...
- After addressing design changes: re-run `/aesir-design-review` on the updated GDD.
- After fixing bugs: re-run `/aesir-bug-triage` to update priorities.
