...
**This skill is the write-time safety net.** It catches what `/aesir-design-system`'s
per-section checks may have missed and what `/aesir-review-all-gdds`'s holistic review
catches too late.

**When to run:**
- After writing each new GDD (before moving to the next system)
- Before `/aesir-review-all-gdds` (so that skill starts with a clean baseline)
- Before `/aesir-create-architecture` (inconsistencies poison downstream ADRs)
- On demand: `/aesir-consistency-check entity:[name]` to check one entity specifically
...
> "Entity registry is empty. Run `/aesir-design-system` to write GDDs — the registry
...
```markdown
### [YYYY-MM-DD] — /aesir-consistency-check — 🔴 CONFLICT
...
<!-- Auto-maintained by /aesir-consistency-check. Do not edit manually. -->
...
- **Options**:
  ...
  - `[C] Run /aesir-design-review on the most conflicted GDD`
  - `[D] Stop here`
...
- **If PASS**: Run `/aesir-review-all-gdds` for holistic design-theory review, or
  `/aesir-create-architecture` if all MVP GDDs are complete.
- **If CONFLICTS FOUND**: Fix the flagged GDDs, then re-run
  `/aesir-consistency-check` to confirm resolution.
- **If STALE REGISTRY**: Update the registry (Phase 6), then re-run to verify.
- Run `/aesir-consistency-check` after writing each new GDD to catch issues early,
