...
# Skill Test Spec: /aesir-consistency-check

## Skill Summary

`/aesir-consistency-check` scans all GDDs in `design/gdd/` and checks for internal
...
- [ ] Skill suggests running `/aesir-design-system system-B` to create the missing GDD
...
3. Skill outputs an error: "No GDDs found in `design/gdd/`. Run `/aesir-design-system` to create GDDs first."
...
- [ ] Skill recommends the correct next action (`/aesir-design-system`)
...
- This skill checks for structural consistency between GDDs. Deep design theory
  analysis (pillar drift, dominant strategies) is handled by `/aesir-review-all-gdds`.
