---
name: aesir-create-architecture
...
**Distinct from `/aesir-architecture-decision`**: ADRs record individual point decisions.
...
> "No engine is configured. Run `/aesir-setup-engine` first. Architecture cannot be
...
3. `aesir-gameworks references/technical-preferences.md` — naming conventions, performance budgets,
...
- `/aesir-architecture-decision [title]` → covers: TR-[id], TR-[id]

**Core Layer:**
- `/aesir-architecture-decision [title]` → covers: TR-[id]
...
Apply gate **TD-ARCHITECTURE** (`aesir-gameworks references/director-gates.md`) as a self-review. Check all four criteria from that gate definition against the completed document.
...
**Step 2 — Spawn `lead-programmer` via Task using gate LP-FEASIBILITY (`aesir-gameworks references/director-gates.md`):**
...
Use `clarify` — "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
**1. `/aesir-architecture-decision "[Title]"` → ADR-[XXXX]**
[One sentence: what it defines and what it unblocks.]

**2. `/aesir-architecture-decision "[Title]"` → ADR-[XXXX]**
[One sentence.]

**3. `/aesir-architecture-decision "[Title]"` → ADR-[XXXX]**
[One sentence.]
...
> **Required before `/aesir-gate-check [stage]`:**
> - [ ] Accept ADRs: [list Proposed ADR IDs that must be Accepted]
> - [ ] Write ADRs: [list ADR IDs that must still be written]
> - [ ] Run `/aesir-test-setup` — scaffolds `tests/unit/`, `tests/integration/`, CI workflow, and an example test file
> - [ ] Run `/aesir-ux-design` — creates `design/ux/interaction-patterns.md` and `design/accessibility-requirements.md`
>
> Run `/aesir-gate-check [stage]` when all boxes are checked.

If nothing is blocking, write instead:
> No blockers — run `/aesir-gate-check [stage]` now.
...
5. **Use `clarify` for write approvals** — plain text "May I?" is not
...
- Run `/aesir-architecture-decision [title]` for each required ADR listed in Phase 6 — Foundation layer ADRs first
- Run `/aesir-architecture-review` — bootstraps the Requirements Traceability Matrix and TR registry from the ADRs just written. Required before the Pre-Production gate.
- Run `/aesir-test-setup` to scaffold `tests/unit/`, `tests/integration/`, CI workflow, and an example test (required for gate-check)
- Run `/aesir-ux-design` to initialize `design/ux/interaction-patterns.md` and `design/accessibility-requirements.md` (required for gate-check)
- Run `/aesir-create-control-manifest` once the required ADRs are written to produce the layer rules manifest
- Run `/aesir-gate-check pre-production` when all required ADRs, `/aesir-test-setup`, and `/aesir-ux-design` are complete
