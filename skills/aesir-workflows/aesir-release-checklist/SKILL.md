...
> **Explicit invocation only**: This skill should only run when the user explicitly requests it with `/aesir-release-checklist`. Do not auto-invoke based on context matching.
...
- Run `/aesir-gate-check` for a formal phase gate verdict before proceeding to release.
- Coordinate final sign-offs via `/aesir-team-release`.
