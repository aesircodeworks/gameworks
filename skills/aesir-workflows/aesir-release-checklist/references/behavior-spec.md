...
# Skill Test Spec: /aesir-release-checklist

## Skill Summary

`/aesir-release-checklist` generates an internal release readiness checklist covering:
...
(that is `/aesir-launch-checklist`). When a previous release checklist exists, it shows
...
after a "May I write" ask. No director gates apply — `/aesir-gate-check` handles
...
- [ ] Has a next-step handoff (e.g., `/aesir-launch-checklist` for external or `/aesir-gate-check` for phase)

---

## Director Gate Checks

None. `/aesir-release-checklist` is an internal audit utility. Formal phase advancement
is managed by `/aesir-gate-check`.
