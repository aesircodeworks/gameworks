...
Check:
- **Engine configured?** Read `aesir-gameworks references/technical-preferences.md`. If the Engine field contains `[TO BE CONFIGURED]`, the engine is not set.
...
This is the first thing the user sees. Use `clarify` with these exact options so the user can click rather than type:

- **Prompt**: "Welcome to Aesir Gameworks! Before I suggest anything, I'd like to understand where you're starting from. Where are you at with your game idea right now?"
- **Options**:
...
2. Briefly explain what `/aesir-brainstorm` does (guided ideation using professional frameworks — MDA, player psychology, verb-first design). Mention that it has two modes: `/aesir-brainstorm open` for fully open exploration, or `/aesir-brainstorm [hint]` if they have even a vague theme (e.g., "space", "cozy", "horror").
3. Recommend running `/aesir-brainstorm open` as the next step, but invite them to use a hint if something comes to mind
4. Show the recommended path:
   **Concept phase:**
   - `/aesir-brainstorm open` — discover your game concept
   - `/aesir-setup-engine` — configure the engine (brainstorm will recommend one)
   - `/aesir-prototype` — throwaway concept build: validate the core idea is fun before designing (1–3 days)
   - `/aesir-art-bible` — define visual identity (uses the Visual Identity Anchor brainstorm produces)
   - `/aesir-map-systems` — decompose the concept into systems
   - `/aesir-design-system` — author a GDD for each MVP system
   - `/aesir-review-all-gdds` — cross-system consistency check
   - `/aesir-gate-check` — validate readiness before architecture work
   **Architecture phase:**
   - `/aesir-create-architecture` — produce the master architecture blueprint and Required ADR list
   - `/aesir-architecture-decision (×N)` — record key technical decisions, following the Required ADR list
   - `/aesir-create-control-manifest` — compile decisions into an actionable rules sheet
   - `/aesir-architecture-review` — validate architecture coverage
   **Pre-Production phase:**
   - `/aesir-ux-design` — author UX specs for key screens (main menu, HUD, core interactions)
   - `/aesir-vertical-slice` — production-quality end-to-end build to validate the full game loop
   - `/aesir-playtest-report (×1+)` — document each vertical slice playtest session
   - `/aesir-create-epics` — map systems to epics
   - `/aesir-create-stories` — break epics into implementable stories
   - `/aesir-sprint-plan` — plan the first sprint
   **Production phase:** → pick up stories with `/aesir-dev-story`
...
3. Recommend running `/aesir-brainstorm [their hint]` to develop it
4. Show the recommended path:
   **Concept phase:**
   - `/aesir-brainstorm [hint]` — develop the idea into a full concept
   - `/aesir-setup-engine` — configure the engine
   - `/aesir-prototype` — throwaway concept build: validate the core idea is fun before designing (1–3 days)
   - `/aesir-art-bible` — define visual identity (uses the Visual Identity Anchor brainstorm produces)
   - `/aesir-map-systems` — decompose the concept into systems
   - `/aesir-design-system` — author a GDD for each MVP system
...
1. Ask them to describe their concept in one sentence — genre and core mechanic. Use plain text, not clarify (it's an open response).
2. Acknowledge the concept, then use `clarify` to offer two paths:
   - **Prompt**: "How would you like to proceed?"
   - **Options**:
     - `Formalize it first` — Run `/aesir-brainstorm [concept]` to structure it into a proper game concept document
     - `Jump straight in` — Go to `/aesir-setup-engine` now and write the GDD manually afterward
3. Show the recommended path:
   **Concept phase:**
   - `/aesir-brainstorm` or `/aesir-setup-engine` — (their pick from step 2)
   - `/aesir-prototype` — throwaway concept build: validate the core idea is fun before designing (1–3 days)
   - `/aesir-art-bible` — define visual identity (after brainstorm if run, or after concept doc exists)
   - `/aesir-design-review` — validate the concept doc
   - `/aesir-map-systems` — decompose the concept into individual systems
   - `/aesir-design-system` — author a GDD for each MVP system
   - `/aesir-review-all-gdds` — cross-system consistency check
   - `/aesir-gate-check` — validate readiness before architecture work
   **Architecture phase:**
...
2. **Sub-case D1 — Early stage** (engine not configured or only a game concept exists):
   - Recommend `/aesir-setup-engine` first if engine not configured
   - Then `/aesir-project-stage-detect` for a gap inventory

   **Sub-case D2 — GDDs, ADRs, or stories already exist:**
   - Explain: "Having files isn't the same as the template's skills being able to use them. GDDs might be missing required sections. `/aesir-adopt` checks this specifically."
   - Recommend:
     1. `/aesir-project-stage-detect` — understand what phase and what's missing entirely
     2. `/aesir-adopt` — audit whether existing artifacts are in the right internal format

3. Show the recommended path for D2:
   - `/aesir-project-stage-detect` — phase detection + existence gaps
   - `/aesir-adopt` — format compliance audit + migration plan
   - `/aesir-setup-engine` — if engine not configured
   - `/aesir-design-system retrofit [path]` — fill missing GDD sections
   - `/aesir-architecture-decision retrofit [path]` — add missing ADR sections
   - `/aesir-architecture-review` — bootstrap the TR requirement registry
   - `/aesir-gate-check` — validate readiness for next phase
...
- **Options**:
  ...
  - `Lean (recommended)` — Directors only at phase gate transitions (/aesir-gate-check). Skips per-skill reviews. Balanced approach for solo devs and small teams.
...
After presenting the recommended path, use `clarify` to ask the user which step they'd like to take first. Never auto-run the next skill.
...
When the user confirms their next step, respond with a single short line: "Type `[skill command]` to begin." Nothing else. Do not re-explain the skill or add encouragement. The `/aesir-start` skill's job is done.
