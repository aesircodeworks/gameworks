...
If STILL PRESENT: reopen the bug, set Status back to Open, and suggest re-running `/aesir-hotfix [BUG-ID]`.

---

## Phase 2D: Close Mode

Read `production/qa/bugs/[BUG-ID].md`. Confirm Status is `Verified Fixed` before closing. If status is anything else, stop: "Bug [ID] must be Verified Fixed before it can be closed. Run `/aesir-bug-report verify [BUG-ID]` first."
...
After closing, check `production/qa/bug-triage-*.md` — if the bug appears in an open triage report, note: "Bug [ID] is referenced in the triage report. Run `/aesir-bug-triage` to refresh the open bug count."
...
- Run `/aesir-bug-triage` to prioritize alongside existing open bugs
- If S1 or S2: run `/aesir-hotfix [BUG-ID]` for emergency fix workflow

**After fixing the bug (developer confirms fix is in):**
- Run `/aesir-bug-report verify [BUG-ID]` — confirm the fix actually works before closing
- Never mark a bug closed without verification — a fix that doesn't verify is still Open

**After verify returns VERIFIED FIXED:**
- Run `/aesir-bug-report close [BUG-ID]` — write the closure record and update status
- Run `/aesir-bug-triage` to refresh the open bug count and remove it from the active list
