...
Then use `clarify` to collect the **category**:
...
Then use `clarify` to collect the **estimated fix effort**:
...
- Run `/aesir-sprint-plan` to schedule high-priority debt items into the next sprint.
- Run `/aesir-tech-debt report` at the start of each sprint to track debt trends over time.
