...
# Skill Test Spec: /aesir-project-stage-detect

## Skill Summary

`/aesir-project-stage-detect` automatically analyzes project artifacts to determine
...
when `/aesir-gate-check` passes and the user confirms advancement. The skill reports
...
- [ ] Has a next-step handoff (e.g., `/aesir-gate-check` to formally advance stage)

---

## Director Gate Checks

None. `/aesir-project-stage-detect` is a read-only detection utility. No director
gates apply.
...
**Input:** `/aesir-project-stage-detect`
...
5. Next step: continue with `/aesir-sprint-plan` or `/aesir-dev-story`
...
5. Skill recommends running `/aesir-gate-check` to formalize and write stage.txt

**Assertions:**
- [ ] Inferred stage is Production
- [ ] Confidence is MEDIUM (not HIGH, since stage.txt is absent)
- [ ] Recommendation to run `/aesir-gate-check` is present
...
5. Skill suggests `/aesir-start` to begin the onboarding workflow

**Assertions:**
- [ ] Inferred stage is Concept
- [ ] Output lists the artifacts that were checked (and found absent)
- [ ] `/aesir-start` is suggested as the next step
- [ ] No files are written
...
5. Skill suggests reviewing stage.txt manually or running `/aesir-gate-check`
...
- [ ] Does not write stage.txt (that is `/aesir-gate-check`'s responsibility)
