---
name: aesir-project-stage-detect
...
if it exists, use its value (explicit override from `/aesir-gate-check`). Otherwise,
...
| **Polish** | Explicit only (set by `/aesir-gate-check` Production → Polish gate) |
| **Release** | Explicit only (set by `/aesir-gate-check` Polish → Release gate) |
...
- "I found a game concept but no systems index. Have you decomposed the concept into individual systems yet, or should we run `/aesir-map-systems`?"
...
Use template: `aesir-project-templates templates/project-stage-report.md`
...
If user provided a role argument (e.g., `/aesir-project-stage-detect programmer`):
...
/aesir-project-stage-detect

# Programmer-focused analysis
/aesir-project-stage-detect programmer

# Designer-focused analysis
/aesir-project-stage-detect designer
```
...
- **Concept exists but no systems index?** → `/aesir-map-systems` to decompose into systems
- **Missing design docs?** → `/aesir-reverse-document design src/[system]`
- **Missing architecture docs?** → `/aesir-architecture-decision` or `/aesir-reverse-document architecture`
- **Prototypes need documentation?** → `/aesir-reverse-document concept prototypes/[name]`
- **No sprint plan?** → `/aesir-sprint-plan`
- **Approaching milestone?** → `/aesir-milestone-review`
