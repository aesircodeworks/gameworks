---
name: aesir-story-readiness
description: Use when running the Aesir story-readiness workflow. Validate that a
  story file is implementation-ready. Checks for embedded GDD requirements, ADR references,
  engine notes, clear acceptance criteria, and no open design questions. Produces
  READY / NEEDS WORK / BLOCKED verdict with specific gaps. Use when user says 'is
  this story ready', 'can I start on this story', 'is story X ready to implement'.
version: 1.0.0
...
See `aesir-gameworks references/director-gates.md` for the full check pattern and mode definitions.

---

## 1. Parse Arguments

**Scope:** `$ARGUMENTS[0]` (blank = ask user via clarify)

- **Specific path** (e.g., `/aesir-story-readiness production/epics/combat/story-001-basic-attack.md`):
...
If no argument is given, use `clarify`:
...
- [ ] **TR-ID is valid and active**: If the story contains a `TR-[system]-NNN`
  ...
  - If the ID does not exist in the registry → NEEDS WORK: ID was not registered
    (story may predate registry, or registry needs an `/aesir-architecture-review` run).
...
Resolve these before the sprint begins or replan with `/aesir-sprint-plan update`.
```
...
`/aesir-create-stories`) handles writing.

**Redirect rules:**
- If a story file does not exist at all: "This story file is missing entirely.
  Run `/aesir-create-epics [layer]` then `/aesir-create-stories [epic-slug]` to generate stories from the GDD and ADR."
- If a story has no GDD reference and the work appears small: "This story has
  ...
  `/aesir-quick-design [description]` to create a Quick Design Spec, then reference
...
Run `/aesir-story-readiness [path]` to validate before starting.
```
...
Spawn `qa-lead` via Task using gate **QL-STORY-READY** (`aesir-gameworks references/director-gates.md`).
...
- **GAPS [list]** → surface the specific gaps to the user via `clarify`:
...
- Run `/aesir-dev-story [story-path]` to begin implementation once the story is READY
- Run `/aesir-story-readiness sprint` to check all stories in the current sprint at once
- Run `/aesir-create-stories [epic-slug]` if a story file is missing entirely
