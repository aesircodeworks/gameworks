...
> Usage: `/aesir-team-polish [feature or area]` — specify the feature or area to polish (e.g., `combat`, `main menu`, `inventory system`, `level-1`). Do not use `clarify` here; output the guidance directly.
...
- Profile the target feature/area using `/aesir-perf-profile`
...
- If READY FOR RELEASE: run `/aesir-release-checklist` for the final pre-release validation.
- If NEEDS MORE WORK: schedule remaining issues in `/aesir-sprint-plan update` and re-run `/aesir-team-polish` after fixes.
- Run `/aesir-gate-check` for a formal phase gate verdict before handing off to release.
