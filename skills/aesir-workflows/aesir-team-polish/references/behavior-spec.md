...
# Skill Test Spec: /aesir-team-polish
...
and issues READY FOR RELEASE or NEEDS MORE WORK). Uses `clarify` at each
...
- [ ] Has a next-step handoff at the end (references `/aesir-release-checklist`, `/aesir-sprint-plan update`, `/aesir-gate-check`)
...
**Input:** `/aesir-team-polish combat`
...
2. `clarify` presents performance report; user approves before Phases 2, 3, and 4 begin
...
5. All three parallel phases complete; `clarify` presents results; user approves before Phase 5 begins
...
**Input:** `/aesir-team-polish particle-storm`
...
2. `clarify` presents the violation; user chooses to proceed with optimization attempt
...
9. Next Steps: schedule the remaining issue in `/aesir-sprint-plan update`; re-run `/aesir-team-polish` after fix
...
- [ ] Next Steps references `/aesir-sprint-plan update` for scheduling the remaining fix
...
**Input:** `/aesir-team-polish` (no argument)

**Expected behavior:**
1. Skill detects no argument is provided
2. Outputs usage guidance: e.g., "Usage: `/aesir-team-polish [feature or area]` — specify the feature or area to polish (e.g., `combat`, `main menu`, `inventory system`, `level-1`)"
...
**Input:** `/aesir-team-polish open-world`
...
3. `clarify` presents the performance report including the engine-level root cause; user approves before Phase 2
...
**Input:** `/aesir-team-polish inventory-ui` (Phase 5 scenario)
...
- [ ] Next Steps handoff references `/aesir-release-checklist` (on success) and `/aesir-sprint-plan update` + `/aesir-gate-check` (on failure)
