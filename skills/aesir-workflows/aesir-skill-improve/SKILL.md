...
```
Usage: /aesir-skill-improve [skill-name]
Example: /aesir-skill-improve tech-debt
```
...
Run `/aesir-skill-test static [name]` and record the baseline score:
- Count of FAILs
- Count of WARNs
...
Look up the skill's `category:` field in `aesir-framework-qa references/catalog.yaml`.

If no `category:` field is found, display:
"Category: not yet assigned — skipping category checks."
and skip to Phase 3.

If category is found, run `/aesir-skill-test category [name]` and record the category baseline:
- Count of FAILs
- Count of WARNs
...
Re-run `/aesir-skill-test static [name]` and record the new static score.
If a category was assigned, also re-run `/aesir-skill-test category [name]` and record the new category score.
...
- Run `/aesir-skill-test static all` to find the next skill with failures.
- Run `/aesir-skill-improve [next-name]` to continue the loop on another skill.
- Run `/aesir-skill-test audit` to see overall coverage progress.
