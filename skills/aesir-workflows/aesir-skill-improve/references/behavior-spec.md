...
# Skill Test Spec: /aesir-skill-improve

## Skill Summary

`/aesir-skill-improve` runs an automated test-fix-retest improvement loop on a skill
file. It invokes `/aesir-skill-test static` (and optionally `/aesir-skill-test category`) to
...
Verified automatically by `/aesir-skill-test static` — no fixture needed.
...
- [ ] Has a next-step handoff (e.g., run `/aesir-skill-test spec` to validate behavioral compliance)

---

## Director Gate Checks

None. `/aesir-skill-improve` is a meta-utility skill. No director gates apply.
...
**Fixture:**
- `skill_view('aesir-some-skill')/SKILL.md` has 2 static failures:
  - Check 4: no "May I write" language despite having Write in Hermes tools
  - Check 5: no next-step handoff at the end

**Input:** `/aesir-skill-improve some-skill`

**Expected behavior:**
1. Skill runs `/aesir-skill-test static some-skill` — baseline: 5/7 checks pass
...
4. Skill asks "May I write improvements to `skill_view('aesir-some-skill')/SKILL.md`?"
5. Fixes applied; `/aesir-skill-test static some-skill` re-run — now 7/7 checks pass
...
**Fixture:**
- `skill_view('aesir-some-skill')/SKILL.md` has 1 static failure (missing handoff)
...
**Fixture:**
- `skill_view('aesir-gate-check')/SKILL.md` is a gate skill with 1 static failure
  and 2 category (G-criteria) failures
- `tests/skills/quality-rubric.md` has Gate Skills section

**Input:** `/aesir-skill-improve gate-check`
...
4. "May I write improvements to `skill_view('aesir-gate-check')/SKILL.md`?"
...
**Fixture:**
- `skill_view('aesir-brainstorm')/SKILL.md` has no static failures
- Category score is also 5/5 (if applicable)

**Input:** `/aesir-skill-improve brainstorm`

**Expected behavior:**
1. Skill runs `/aesir-skill-test static brainstorm` — 7/7 checks pass
...
- The improvement loop is designed to run only one fix-retest cycle per
  invocation; running multiple iterations requires re-invoking `/aesir-skill-improve`.
