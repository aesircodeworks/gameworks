---
name: aesir-architecture-review
...
### Project Standards
- `aesir-gameworks references/technical-preferences.md`
...
For each gap:
  ❌ TR-[id]: [GDD] → [system] → [requirement]
     Suggested ADR: "/aesir-architecture-decision [suggested title]"
...
> Last Updated: [date]
> Mode: /aesir-architecture-review rtm
...
```markdown
### [YYYY-MM-DD] — /aesir-architecture-review — 🔴 CONFLICT
...
`production/session-state/active.md`:
    ...
    ## Session Extract — /aesir-architecture-review [date]
    - Verdict: [PASS / CONCERNS / FAIL]
...
2. **Pre-gate checklist**: Check whether these exist via Glob and mark each ✅ or ❌:
   - `tests/unit/` and `tests/integration/` directories — if ❌: run `/aesir-test-setup`
   - `.github/workflows/tests.yml` — if ❌: run `/aesir-test-setup`
   - `design/accessibility-requirements.md` — if ❌: run `/aesir-ux-design`
   - `design/ux/interaction-patterns.md` — if ❌: run `/aesir-ux-design`
   Present ❌ items as required steps before gate-check. Do not offer `/aesir-gate-check`
   as an option if any item is ❌ — offer the missing skill to run instead.
3. **Rerun trigger**: "Re-run `/aesir-architecture-review` after each new ADR is written
...
- If ADR gaps remain or any pre-gate item is ❌:
  - "Architecture review complete. What would you like to do next?"
    - [A] Write a missing ADR — open a fresh session and run `/aesir-architecture-decision [system]`
    - [B] Run `/aesir-test-setup` — required before gate-check (only show if test infrastructure is ❌)
    - [C] Run `/aesir-ux-design` — required before gate-check (only show if UX/accessibility files are ❌)
...
- If all pre-gate checklist items are ✅ and no blocking ADR gaps remain:
  - "Architecture review complete. All pre-gate items confirmed. What would you like to do next?"
    - [A] Run `/aesir-gate-check pre-production`
    - [B] Write a missing ADR — open a fresh session and run `/aesir-architecture-decision [system]`
...
3. **Offer options** via clarify with three choices:
