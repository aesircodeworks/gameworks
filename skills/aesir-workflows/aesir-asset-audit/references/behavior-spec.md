...
# Skill Test Spec: /aesir-asset-audit

## Skill Summary

`/aesir-asset-audit` audits the `assets/` directory for naming convention compliance,
...
**Expected behavior:**
1. Skill reads GDD references to find expected assets (cross-references with `/aesir-content-audit` scope)
...
- The interaction between `/aesir-asset-audit` and `/aesir-content-audit` (both check GDD
  references vs. assets) is intentional overlap; `/aesir-asset-audit` focuses on
  compliance while `/aesir-content-audit` focuses on completeness.
