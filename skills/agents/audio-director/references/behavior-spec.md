...
**Model tier:** Medium (individual system analysis — audio direction and spec review).
...
Verified by reading the agent's `skill_view('audio-director')` frontmatter:
...
- [ ] `Hermes tools:` list is read-focused; no terminal unless audio asset pipeline checks are justified
