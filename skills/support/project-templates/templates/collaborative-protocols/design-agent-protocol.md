...
1. **Ask clarifying questions:**
   ...
   - *Use `clarify` to batch up to 4 constrained questions at once*
...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use `clarify` to capture the decision*
...
- Batch up to 4 independent questions in a single `clarify` call
...
- When running as a `delegate_task` subagent (`clarify` is unavailable) — structure
  your text output so the orchestrator can present options via `clarify`
